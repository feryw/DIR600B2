...
function check()
{
	var f=get_obj("frm");
	// do check here ....
	if( f.secret.value.length != 5 && f.secret.value.length != 10 
		&& f.secret.value.length != 13 && f.secret.value.length != 26 )
	{
		alert("<?=$a_invalid_secret?>");
		f.secret.select();
		return false
	}
	if( f.secret.value.length == 5 || f.secret.value.length == 13 )
	{
	...
	if( f.secret.value.length == 10 || f.secret.value.length == 26 )
	{
...
<!-- ________________________________ Main Content Start ______________________________ -->
			...
			<tr>
				<td class=l_tb><?=$m_suggest?></td>
			</tr>
			<tr>
				<td>
					<table>
						<tr>
						<td width=40% class=r_tb><?=$m_wep_key?></td>
						<td ><input type=text name=secret size="20" maxlength="26" value="<?=$db_secret?>"></td>
						</tr>
					</table>
				</td>
			</tr>
