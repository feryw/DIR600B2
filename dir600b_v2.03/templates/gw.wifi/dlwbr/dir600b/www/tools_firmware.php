...
if(query("/runtime/func/checkfw") == "1"){
	if ($ACTION_POST!=""){


	require("/www/model/__admin_check.php");
	$db_dirty=0;
	if(query("/sys/email/firmwarenotify")!=$firmwarenotify){set("/sys/email/firmwarenotify",$firmwarenotify);$db_dirty++;}
	if(query("/sys/autocheckfirmware")!=$autocheckfirmware){set("/sys/autocheckfirmware",$autocheckfirmware);$db_dirty++;}
	
	if($db_dirty>0 && $SUBMIT_STR==""){$SUBMIT_STR="submit FIRMWARE";}
	$NEXT_PAGE=$MY_NAME;
...
if(query("/runtime/func/checkfw") == "1"){
	$firmwarenotify	= query("/sys/email/firmwarenotify");
	if ($firmwarenotify != 1){$firmwarenotify=0;}

	$autocheckfirmware	= query("/sys/autocheckfirmware");
	if($autocheckfirmware !=1){$autocheckfirmware=0;}

	$havenewfirmware = query("/runtime/havefirmware");
	if($havenewfirmware !=1){$havenewfirmware=0;}
}
...
// <--------- firmware autocheck
function have_new_fw_needdisply()
{
...
function update_fw_info_for_display()
{
	if (AjaxReq != null && AjaxReq.readyState == 4)
	{
		if (AjaxReq.status == 200)
...
function send_fw_check_req_disply()
{
	if (AjaxReq != null )
	{
			delete AjaxReq;
			...
			// Read data from fwinfo.xml
	var address = "/var/firmware.xml?date="+generate_random_str();
	AjaxReq = createRequest();
	AjaxReq.open("GET", address, true);
	AjaxReq.onreadystatechange = update_fw_info_for_display;
	AjaxReq.send(null);
	
}

function send_lang_check_disply()
{
	if (AjaxReq != null )
	{
			delete AjaxReq;
	...
	var address = "/var/langpack.xml?date="+generate_random_str();
	AjaxReq = createRequest();
	AjaxReq.open("GET", address, true);
	AjaxReq.onreadystatechange = update_lang_info;
	AjaxReq.send(null);
...
function init(){
	...
	if(query("/runtime/func/checkfw") == "1")
	{
		...
		echo "\tif(".$firmwarenotify."==1){\n";
			echo "\t\tf.fmwnotify.checked=true;\n";
		echo "\t}\n";

		echo "\tif(".$autocheckfirmware."==1){\n";
			echo "\t\tf.autocheckfmw.checked=true;\n";
			echo "\t\t\tif(".$havenewfirmware."==1){\n";
				echo "\t\t\t\thave_new_fw_needdisply();\n";	
			echo "\t\t\t}\n";
		echo "\t}\n";
		
		if(query("/sys/autocheckfirmware") == "1")
		{
			if(query("/runtime/firmware/havenewfirmware") == "1")
			{
				echo "send_fw_check_req_disply();\n";
			}
		}
	}
	?>
...
if(query("/runtime/func/checkfw") == "1")
{
echo "function submit_notify()
{	
	var sub=get_obj(\"notify\");

	sub.firmwarenotify.value=1;

	var f=get_obj(\"frm\");

	if(f.fmwnotify.checked == true)
	{
			
		sub.firmwarenotify.value=1;
	}else
	{
		sub.firmwarenotify.value=0;
	}

	if(f.autocheckfmw.checked == true)
	{
		sub.autocheckfirmware.value=1;
	}
	else
	{
		sub.autocheckfirmware.value=0;
	}
	sub.submit();
}";	
...
<tr valign=top>
	...
	<td id="maincontent">
		...
		<? if(query("/runtime/func/checkfw") == "1"){
			echo "<script>apply('submit_notify()'); echo (\"&nbsp;\"); cancel('');</script>";
		}?>
		</div>
		...
		<?
			if(query("/runtime/func/checkfw") == "1")
			{
				...
				echo "	\t\t<h2>".$m_notify."</h2>\n";
					...
					echo "		\t\t<tr>\n";
					echo "			\t\t<td class=br_tb width=230>".$m_autocheck." :</td>\n";
					echo "			\t\t<td><input type=checkbox name=autocheckfmw size=30 value=1></td>\n";
					echo "		\t\t</tr>";
					echo "		\t\t<tr>";
					echo "			\t\t<td class=br_tb width=230>".$m_email_notify." :</td>\n";
					echo "			\t\t<td><input type=checkbox name=fmwnotify size=30 value=1></td>\n";
					echo "		\t\t</tr>\n";
					echo "	\t\t</table>\n";
				echo "	\t</div>\n";
			}		
...
if(query("/runtime/func/checkfw") == "1")
{
	echo "<form name=\"notify\" id=\"notify\" method=\"post\" action=\"".$MY_NAME.".php\" >\n";
		echo "<input type=\"hidden\" name=\"ACTION_POST\" value=\"SOMETHING\">\n";
		echo "<input type=\"hidden\" name=\"firmwarenotify\" value=\"\">\n";
		echo "<input type=\"hidden\" name=\"autocheckfirmware\"	value=\"\">\n";
	echo "</form>\n";
}		
?>
