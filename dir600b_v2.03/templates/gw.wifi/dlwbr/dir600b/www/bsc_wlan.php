...
if ($ACTION_POST!="")
{
	...
	if($f_enable=="1")
	{
		...
		if(query("schedule/id")	!= $f_wlan_sch)			{set("schedule/id",	$f_wlan_sch);		$db_dirty++;}
	...
	if($db_dirty > 0)	{set("/sys/configdirty", "1");$SUBMIT_STR="submit WLAN";}
	$NEXT_PAGE=$MY_NAME;
...
$cfg_wlan_sch	= query("schedule/id");//gold
$cfg_ssid		= get("j", "ssid");
...
<script>
<?
require("/www/__bsc_wlan_sch.php");
?>
...
function on_check_enable()
{
	...
	if (f.enable.checked)
	{
		...
		f.wlan_sch.disabled = false;
		f.wlan_sch_btn.disabled = false;
...
{
	var now_time = new Array();
	var arys= new Array();
	arys = cfg_date.split('/');
	cur_date = new Date(arys[2],arys[1]-1,arys[0]);
	now_time[0] = cur_date.getDay();
	now_time[1] = new Array();
	arys = cfg_time.split(':');
	now_time[1][0] = arys[0];
	now_time[1][1] = arys[1];
	return now_time;
...
function check()
{
	...
	f_final.f_wlan_sch.value		=f.wlan_sch.value;
...
function check_time()
{
	...
	var time1 = new Array();
	var result = false;
	var day = now[0] == 7?0:now[0];
	if(f.wlan_sch.selectedIndex == 0) 
	{
		return true;
	}
	if((day_sch[f.wlan_sch.selectedIndex] & (1 << day)) == 0)
	{
		return false;
	}
	time0 = time_sch[f.wlan_sch.selectedIndex][0].split(':');
	time1 = time_sch[f.wlan_sch.selectedIndex][1].split(':');
	if(now[1][0] < time0[0] || now[1][0] > time1[0])
	{
		return false;
	}
	if(now[1][0] == time0[0] && now[1][1] < time0[1] || now[1][0] == time1[0] && now[1][1] >= time1[1])
	{
		return false;
	}
	return true;
...
function drawRateList( id, type )
{
	...
	else
	{
		if( wlanMode == "4" )
		{
			...
			/* kwest: RT3050 is 1T1R wireless chip, only support MCS 0~7 */
			switch( condi )
			{
			case "10":
...
<input type="hidden" name="f_wlan_sch"			value="">
...
<?						$PREFIX		= "\t\t\t\t\t";//gold
						$OBJID		= "wlan_sch";
						$OBJNAME	= "wlan_sch";
						$UNIQUEID	= $cfg_wlan_sch;
						$ON_CHANGE_FUNC	= "check_sch()";
...
<?if(query("/wireless/wifi_test")!=1) { 
	if($security_type!=2 && $security_type!=3) { 
						echo "<!--\n"; }}?>
			...
			<div>
				...
				<tr>
					...
					<?=$td2?><?=$symbol?>
						...
						<option value="4"><?=$m_tkipandaes?></option>
