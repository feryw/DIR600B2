...
if ($router=="1")
{
	...
	else if ($ACTION_POST!="")
	{
		...
		if($unique_id=="")
		{
			...
			{	if ($id_used==0 && query("enable")==1 && query("schedule/id")==$uid) {$id_used=1;}	}
			if ($id_used>0) { $SUBMIT_STR=$SUBMIT_STR."; submit RG_POLICY"; }
			/* DMZ */
			...
			if (query("/sys/log/enablemailschedule")==1 && query("/sys/log/schedule/id")==$uid)
			{
				$SUBMIT_STR=$SUBMIT_STR."; submit SYSLOG";
			}
			if (query("/wireless/enable")==1 && query("/wireless/schedule/id")==$uid)
			{
				$SUBMIT_STR=$SUBMIT_STR."; submit WLAN";
			}
...
for("/security/policy/entry")			{	$uid=query("schedule/id"); if($uid!=""){echo ",'".$uid."'";}	} echo "\n";
...
$uid = query("/wireless/schedule/id"); 			if ($uid!="") { echo ",'".$uid."'"; } echo "\n";
...
function check()
{
	...
	if(days==7 && f.sch_time_start.value=="00:00" && f.sch_time_end.value=="23:59")
	{
		...
		if(!confirm("<?=$a_same_sch_as_always?>"))
			return false;
	}
