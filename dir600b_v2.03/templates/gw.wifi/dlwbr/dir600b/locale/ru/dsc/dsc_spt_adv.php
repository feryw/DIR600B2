...
Функция QoS Engine позволяет улучшить производительность сети путем приоритезации трафика приложений.<br>

<strong>Установка QoS Engine</strong><br>
<strong><em>Включить QoS Engine</em></strong> - Включение этой опции обеспечивает лучшую производительность и качество при работе с online-играми и другими интерактивными приложениями, например, VoIP. <br>
<strong><em>Автоматическая скорость Uplink</em></strong> -  При включении этой опции Маршрутизатор будет автоматически измерять полезную полосу пропускания uplink каждый раз при переустановке WAN-соединения ( например, после перезагрузки).<br>
<strong><em>Измеренная скорость Uplink</em></strong> - Это скорость uplink, измеренная при последней переустановке WAN-интерфейса. Это значение может быть ниже, заявленного провадером, поскольку оно не включает служебную информацию сетевых протоколов в сети провайдера. обычно это значение составляет 87% - 91% указанной скорости uplink для xDSL-соединений и примерно на 5Кбит/с меньше для кабельных соединений.<br>
<strong><em>Скорость Uplink вручную</em></strong> -  Если опция Автоматическая скорость Uplink отключена, то с помощью данной опции можно установить скорость uplink вручную. Скорость Uplink - это скорость, с которой могут передаваться данные от маршрутизатора к провайдеру. Она определяется провайдером. Часто провайдеры задают скорость для исходящего/восходящего потоков; например, 1,5Мбит/с / 284 Кбит/с. Для данного примера необходимо ввести "284". В качестве альтернативы можно проверить скорость uplink с помощью такого сервиса, как www.dslreports.com. Однако следует помнить, что такие сайты как DSL Reports не учитывают служебную информацию сетевых протоколов, поэтому скорость будет несколько меньше, чем Измеренная скорость Uplink или скорость, указанная провайдером<br>
<strong><em>Тип соединения</em></strong> -  По умолчанию маршрутизатор автоматически определяет, принадлежит ли основное соединение сети xDSL/Frame-relay или используется другой тип соединения (например, кабельный модем или Ethernet). Результат будет отображен как Обнаруженная сеть xDSL или Frame Relay. Если используется нестандартное сетевое подключение, например, подключение xDSL, для которого в WAN-настройках выбрана опция "Static" или "DHCP", установка этой опции в значение xDSL или другая сеть Frame Relay обеспечивает то, что маршрутизатор будет формировать трафик несколько иным способом, чтобы обеспечить лучшую производительность. Выбор xDSL или другая сеть Frame Relay приведет к тому, что измеренная скорость uplink будет несколько меньше, чем ранее для данного соединения, но даст несколько лучший результат.<br>
<strong><em>Обнаруженная сеть xDSL или Frame Relay</em></strong> - Здесь отображается Тип соединения, определенный автоматически.<br>
 </td>
 </tr>
 <?if(query("/runtime/func/qos_engine")!="1"){echo "-->\n";}?>
...
<tr>
	<td height=99>
		...
		<p>
			...
			ранее расписания или кликните по кнопке <strong>Новое расписание</strong>, чтобы добавить нове расписание.
		</p>
		<p>
