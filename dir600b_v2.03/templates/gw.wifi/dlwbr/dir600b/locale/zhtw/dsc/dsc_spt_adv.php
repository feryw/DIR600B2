...
<tr>
 ...
 QoS Engine 機制可透過設定應用程式的網路優先使用順序來提升您的網路效能。<br>
<strong>QoS Engine 設定</strong><br>
<strong><em>啟用 QoS Engine</em></strong> - 啟用此項功能可以提升您網路效能並讓特殊應用如線上遊戲或是多媒體影音應用優先使用網路。<br>
<strong><em>自動上傳速率</em></strong> - 當起用起功能,寬頻路由器會自動偵測可用的上傳頻寬,每當寬頻路由器重新啟動皆會重新偵測(例如:重新開機.<br>
<strong><em>測量上傳速率</em></strong> - 當WAN端介面重新連線,此項功能會重新測量上傳速率。相關數據可能會低於ISP業者所提供給您的頻寬因為此測量不包含所有您ISP業者所提供的網路協定相關流量。原則上測量的數據約為xDSL ISP業者提供之87%~91%頻寬數據與比Cable寬頻低5kpbs左右之數據。<br>
<strong><em>手動設定上傳頻寬</em></strong> - 假如自動偵測上傳速率為停用，此選項讓您可以手動設定上傳速率。上傳速率指的是您的寬頻路由器連線IPS網路的頻寬。寬頻的大小是取決於ISP業者。IPS業者通常會提供您下載/上傳兩種頻寬數據。例如1.5Mbps/284kbps,在這範例中表示您必須輸入284kbps上傳頻寬。您也可以透過網路上提供的上傳速率測試程式測試您的上傳頻寬。所有網路上偵測的數據皆為參考資料,實際速率仍以ISP業者提供的資料為主。<br> 
<strong><em>連線類型</em></strong> - 預設寬頻路由器為自動偵測機制因為無法確認您的連線是採用xDSL/Frame-relay 網路或其他連線類型(例如Cable modem 或乙太網路),且偵測的結果會顯示xDSL或Frame Relay網路。假如您的WAN端對外連線方式是設定為"ststic"或"DHCP"方式,針對xDSL或Frame Relay 網路設定此選項將可以得到最交的網路效能。選擇xDSL或Frame Relay 網路雖然透過偵測上傳頻寬所得到的速率比實際連線速率低,但所提供的為最佳的參考值。<br>
<strong><em>偵測xDSL 或Frame Relay 網路</em></strong> - 當連線類型為設定自動偵測,則自動偵測的連線類型將會顯示在這裡。<br>
 </td>
 </tr>
 <?if(query("/runtime/func/qos_engine")!="1"){echo "-->\n";}?>
<tr><td height=20>&nbsp;</td></tr>
