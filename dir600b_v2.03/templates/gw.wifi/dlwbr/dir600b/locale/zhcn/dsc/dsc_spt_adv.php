...
<strong><em>协议类型 </em></strong> - 使用程序的协议。</td>
 ...
 <tr>
 <td height=64> <strong>QoS引擎<font size=4><b><a name=01></a></b></font> </strong><br>
 QoS引擎功能可通过提升应用程序的优先级来获得更佳的网络性能。<br>

<strong>QoS引擎设置</strong><br>
<strong><em>启用QoS引擎</em></strong> - 启用该选项可使在线游戏获得更好的性能和体验，也包括其他一些互动程序，如VoIP。<br>
<strong><em>自动上行链接速率</em></strong> - 当启用时，每次重建WAN接口时(例如重启以后)，该选项会让路由器自动衡量可用的上行链接带宽。<br>
<strong><em>测量上行链接速率</em></strong> - 当前有WAN接口建立时，可测量上行链接的速率。由于您的ISP网络没有启用所有相关的网络协议，因此所测得的速率可能会低于ISP标称的速率。如使用xDSL链接，则测量的上行速率通常是标称速率的87%到91%之间，如是线缆网络连接，则速率会低约5kbps左右。<br>
<strong><em>手动上行链接速率</em></strong> - 如果自动上行链接速率被禁用，则可以通过该选项来手动设置上行链接速率。上行链接速率指的是数据从路由器传输的您的ISP的速度，这个速度取决于您的ISP，通常ISP会成对设定上/下行速率，如ISP给出的参数是1.5Mbps/284kbps，您就要输入“284”。您可以选择在www.dslreports.com中测试您的上行链接速率。需要注意的是，由于没有加载多数网络协议，因此在诸如DSL Reports网站中测得的速率，通常会低于Measured Uplink Speed程序测得的速率或ISP标称的速率。<br> 
<strong><em>连接类型</em></strong> - 默认情况下，路由器会自动指定所用连接类型为xDSL/帧中继网络或某些其他连接类型(如电缆Modem或以太网)，并将结果显示为检测到xDSL或帧中继网络。如果您的网络连接比较特殊，虽然实际上是通过xDSL连接，但在WAN设置中配置为“Static”或“DHCP”，则将该选项设置为xDSL或其他帧中继网络，以确保路由器能识别，并对流量进行轻微的修整，从而获得最佳的性能。选择xDSL或其他帧中继网络，会导致测量所得的上行链接速率略低于之前采用的连接，但实际上会更好。<br>
<strong><em>检测到xDSL或帧中继网络</em></strong> - 当连接类型设置为自动检测时，自动检测到的连接类型会显示在这里。<br>
 </td>
 </tr>
 <?if(query("/runtime/func/qos_engine")!="1"){echo "-->\n";}?>
 <tr>
