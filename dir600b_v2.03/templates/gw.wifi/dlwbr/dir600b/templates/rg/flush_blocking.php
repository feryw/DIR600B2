...
if (query("/security/urlblocking/enable")==1)
{
	...
	if (query("/security/urlblocking/action")=="1")
	{
		...
		echo $IPTURLCMD." -m string --http_req -j HIJACK --to-url ".$LAN_IP."/url_blocking_info.php\n";
		$urlcount++;
	}
	...
	else
	{
		for ("/security/urlblocking/entry")
		{
			...
			if ($target_url!="" && $enable!=0)
			{
				...
				echo $IPTURLCMD.$TIMESTRING." -m string --url \$noHttpUrl -j HIJACK --to-url ".$LAN_IP."/url_blocking_info.php\n";
				$urlcount++;
			}
		}
		echo "logger -p 192.0 \"SYS:011\"\n";
