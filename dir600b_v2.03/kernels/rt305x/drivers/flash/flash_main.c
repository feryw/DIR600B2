...
/* This variable can be used to provide extra debug information
 * when flash programming fails (for ex. the current flash address).
 * This variable is exported via the flash.h file. */
uint32_t flashFailure;
...
uint32_t logic2phy(uint32_t addr)
{
	...
	if (addr >= 0x400000)	return (FL_BASE - 0x400000 + addr);
	else					return (FL_BASE + 0x400000 + addr);
#elif defined (CONFIG_RT2880_FLASH_16M)
	if (addr >= 0x400000)	return (FL_BASE - 0x400000 + addr);
	else					return (FL_BASE + 0xc00000 + addr);
#else
	return addr + FL_BASE;
#endif
...
static uint16_t isSSTFlash(void)
{
	...
	if (man_id==SST_FL_MANUFACT) return 1;
	else return 0;
}
#endif 
...
int32_t FlashSectSize(uint32_t sector_num)
{
	...
	for (i=0;i< MAX_SECTOR_GROUPS;i++)
	{
		...
		if (sector_num < boundary) return (flsh_entry->sector[i].size);
    }
...
// NAME           FlashSectAddr()
//
// SYNOPSIS       int32_t FlashSectAddr(uint32_t sector_addr)
//
// TYPE           Regular function
//
...
int32_t FlashSectNum(uint32_t sector_addr)
{
	...
	for (i=0;i< MAX_SECTOR_GROUPS;i++)
	{
		for (j=0;j<flsh_entry->sector[i].count;j++)
		{
			...
			if (sector_addr < offset) return sector_num;
			sector_num++;
		}
	}
	return -1;
...
int32_t FlashGetSector(uint32_t addr, uint32_t *sect_num)
{
	...
	for (i=0;i< MAX_FLSH_TBL;i++)
	{
		...
		if (flsh_tbl[i].man_id == man_id && flsh_tbl[i].dev_id == dev_id)
		{
