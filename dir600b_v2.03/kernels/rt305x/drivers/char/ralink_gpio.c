 ...
 ***************************************************************************
 *
 * $Id: ralink_gpio.c,v 1.7 2008-01-07 02:52:26 winfred Exp $
 */
...
/* bouble-20090724 - support slic. */
#define ALPHA_SLIC_SUPPORT
#ifdef ALPHA_SLIC_SUPPORT

static int (*ralink_gpio_slic_isr_callback)(unsigned int irq);

/*
 *  Description:
 *    API for slic module to register a callback function
 *  Input:
 *    slic_isr_cb --- pointer to the callback function
 *  Output:
 *    int     --- 0: valid, -EIO: invalid
 */
int ralink_gpio_slic_isr_callback_register(void (*slic_isr_cb)(unsigned int irq))
{
    int error =0;

    if ( slic_isr_cb && ralink_gpio_slic_isr_callback == NULL )
        ralink_gpio_slic_isr_callback = slic_isr_cb;
    else
        error = -EIO;
...
/*
 *  Description:
 *    API for slic module to unregister a callback function
 *  Input:
 *    slic_isr_cb --- pointer to the callback function
 *  Output:
 *    int     --- 0: valid, -EIO: invalid
 */
int ralink_gpio_slic_isr_callback_unregister(void (*slic_isr_cb)(unsigned int irq))
{
    int error =0;

    if ( slic_isr_cb && ralink_gpio_slic_isr_callback == slic_isr_cb )
        ralink_gpio_slic_isr_callback = NULL;
    else
        error = -EIO;
...
EXPORT_SYMBOL(ralink_gpio_slic_isr_callback_register);
EXPORT_SYMBOL(ralink_gpio_slic_isr_callback_unregister);
#endif
		...
		unsigned long arg)
{
	...
	ralink_gpio_reg_info info;
	// David Hsieh: this variable is unused.
...
/* bouble-20090724 - support slic. */
#ifdef ALPHA_SLIC_SUPPORT
	case RALINK_GPIO_RISING_EDGE_ENA_INT:
		...
		*(volatile u32 *)(RALINK_REG_PIORENA) = cpu_to_le32(tmp);
			...
			tmp = le32_to_cpu(*(volatile u32 *)(RALINK_REG_PIORENA));
			printk("RALINK_GPIO_RISING_EDGE_ENA_INT [%x]\n", tmp);
		}
		else 
			return -EINVAL;
		break;
...
#endif
#ifdef CONFIG_RALINK_GPIOMODE_SDRAM
	gpiomode |= RALINK_GPIOMODE_SDRAM;
#endif
#ifdef CONFIG_RALINK_GPIOMODE_RGMII
	gpiomode |= RALINK_GPIOMODE_RGMII;
#endif
...
#ifdef CONFIG_RALINK_GPIOMODE_I2C
	*(volatile u32 *)(RALINK_REG_GPIOMODE) &= ~RALINK_GPIOMODE_I2C;
#endif
#ifdef CONFIG_RALINK_GPIOMODE_SPI
	*(volatile u32 *)(RALINK_REG_GPIOMODE) &= ~RALINK_GPIOMODE_SPI;
#endif
#ifdef CONFIG_RALINK_GPIOMODE_UARTF
	*(volatile u32 *)(RALINK_REG_GPIOMODE) &= ~RALINK_GPIOMODE_UARTF;
#endif
#ifdef CONFIG_RALINK_GPIOMODE_UARTL
	*(volatile u32 *)(RALINK_REG_GPIOMODE) &= ~RALINK_GPIOMODE_UARTL;
#endif
#ifdef CONFIG_RALINK_GPIOMODE_JTAG
	*(volatile u32 *)(RALINK_REG_GPIOMODE) &= ~RALINK_GPIOMODE_JTAG;
#endif
#ifdef CONFIG_RALINK_GPIOMODE_MDIO
	*(volatile u32 *)(RALINK_REG_GPIOMODE) &= ~RALINK_GPIOMODE_MDIO;
#endif
#ifdef CONFIG_RALINK_GPIOMODE_SDRAM
	*(volatile u32 *)(RALINK_REG_GPIOMODE) &= ~RALINK_GPIOMODE_SDRAM;
#endif
#ifdef CONFIG_RALINK_GPIOMODE_RGMII
	*(volatile u32 *)(RALINK_REG_GPIOMODE) &= ~RALINK_GPIOMODE_RGMII;
#endif
...
#endif
{
	...
	for (i = 0; i < RALINK_GPIO_NUMBER; i++) {
		...
		/* bouble- 20090724 - add for slic interrupt */
		#ifdef ALPHA_SLIC_SUPPORT
		if ( ralink_gpio_slic_isr_callback )
		{
			/* if irq match slic irq, then callback should return 1, else return 0 */
			if ( ralink_gpio_slic_isr_callback(ralink_gpio_irqnum)  == 1 )
			{
				/* Debug slic int code */
				#if 0
				u32 ralink_gpio_intp_test = 0;
				u32 ralink_gpio_data_test = 0;

				ralink_gpio_data_test = le32_to_cpu(*(volatile u32 *)(RALINK_REG_PIODATA));
				/* If slic have INT and pull low to active , now*/
				if ( ! (ralink_gpio_data_test & ( 1 << ralink_gpio_irqnum)) )
				{
					ralink_gpio_intp_test = le32_to_cpu(*(volatile u32 *)(RALINK_REG_PIOINT));
					/* Now, INT status register should have this INT, else it have error. */
					if ( ! (ralink_gpio_intp_test & ( 1 << ralink_gpio_irqnum)) )
					{
						printk("Shit ERROR - slic pull low and INT status is wrong. [0x%08x][0x%08x]\n", ralink_gpio_data_test,ralink_gpio_intp_test );
						/* rt3052 just only support 24 interrupt, so use 31 as index temperory .*/
						ralink_gpio_slic_isr_callback(31);
					}
				}
				#endif
