	...
	OUT	USHORT			*FreeNumber)
{
	...
	pWirelessPacket = &pHTTXContext->TransferBuffer->WirelessPacket[fillOffset];
	...
	IN	USHORT			TxIdx)
{
	...
	{		
		...
		if (((pHTTXContext->ENextBulkOutPosition == pHTTXContext->CurWritePosition) || ((pHTTXContext->ENextBulkOutPosition-8) == pHTTXContext->CurWritePosition))
			...
			pWirelessPacket = (PUCHAR)(&pHTTXContext->TransferBuffer->WirelessPacket[fillOffset]);
		...
		//	Zero the last padding.
		//
		pWirelessPacket = (&pHTTXContext->TransferBuffer->WirelessPacket[fillOffset + pTxBlk->Priv]);
		NdisZeroMemory(pWirelessPacket, padding + 8);
	...
	IN UINT 			SrcBufLen)
{
	...
	//for debug
	//hex_dump("RtmpUSBMgmtKickOut", &pMLMEContext->TransferBuffer->WirelessPacket[0], (pMLMEContext->BulkOutSize > 16 ? 16 : pMLMEContext->BulkOutSize));
	...
	IN UINT32		frameLen)
{
	if (pAd->NullContext.InUse == FALSE)
	{
		...
		pWirelessPkt = (PUCHAR)&pNullContext->TransferBuffer->WirelessPacket[0];
