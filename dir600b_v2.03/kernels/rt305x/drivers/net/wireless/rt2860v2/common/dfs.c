...
{
	INT	i;
	UINT32 RadarSignalDuration, RadarSignalDurationEx, RadarDetected;
	...
	else
	{
		...
		for (loop=0; loop < limit; loop++)
		{
			...
			if (Value)
			{
				if ((pAd->CommonCfg.RadarDetect.RDDurRegion == FCC) && (pAd->CommonCfg.HtCapability.HtCapInfo.ChannelWidth  == BW_40) && (loop >= (limit / 2)))
				{
					if (validCntEx >= 3) 
					{
						continue;
...
#ifdef DFS_DEBUG
	if (pAd->CommonCfg.McuRadarDebug & RADR_DONT_SWITCH)
	{
	...
	IN PUCHAR arg)
{
	...
	pAd->CommonCfg.CarrierDetect.CarrierDebug = 1;
