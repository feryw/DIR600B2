	...
	IN	PRTMP_ADAPTER	pAd)
{
	...
	//
	for (i = 0; i < MAX_BARECI_SESSION; i++)
	{
		UCHAR index;
		PRTMP_REORDERBUF	pReorderBuf;

		pAd->LocalRxReorderBuf[i].InUse = FALSE;

		for (index = 0; index < MAX_RX_REORDERBUF; index++)
		{
			pReorderBuf = &pAd->LocalRxReorderBuf[i].MAP_RXBuf[index];

			//buffer for Per MPDU. SO it's 4096bytes. Not whole Aggregated BUlkIn.
			pReorderBuf->AllocSize = RX_BUFFER_NORMSIZE;

			Status = RTMPAllocateMemory((PVOID *)(&pReorderBuf->AllocPa), RX_BUFFER_NORMSIZE);
			if ((Status != NDIS_STATUS_SUCCESS) || (pReorderBuf->AllocPa == NULL))
			{
				Status = NDIS_STATUS_RESOURCES;
				goto out2;
			}

			NdisZeroMemory(pReorderBuf->AllocPa, RX_BUFFER_NORMSIZE);
...
out2:
	...
	for (i = 0; i < MAX_BARECI_SESSION; i++)
	{
		...
		{
			pReorderBuf = &pAd->LocalRxReorderBuf[i].MAP_RXBuf[index];
			if (pReorderBuf->AllocPa != NULL)
			{
				NdisFreeMemory((pReorderBuf->AllocPa), RX_BUFFER_NORMSIZE, 0);
			}
...
//	RTMP_TX_RING		*pTxRing;
	...
	for(i=0; i<MAX_BARECI_SESSION; i++)
	{
		...
		for(acidx=0; acidx<MAX_RX_REORDERBUF; acidx++)
		{
			pReorderBuf = &pAd->LocalRxReorderBuf[i].MAP_RXBuf[acidx];
...
Return Value:
	wireless statistics, can not be NULL or kernel will panic
...
struct iw_statistics *rtusb_get_wireless_stats(IN struct net_device *net_dev)
{
	...
	pAd->iw_stats.qual.noise = R66; // noise level (dBm)
...
static void rt2870_null_frame_complete_tasklet(unsigned long data)
{
	...
	else	// STATUS_OTHER
	{
			...
			(!RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_BULKOUT_RESET)))
		{
			...
			RTMP_SET_FLAG(pAd, fRTMP_ADAPTER_BULKOUT_RESET);
			pAd->bulkResetPipeid = MGMTPIPEIDX;
			RTMP_IRQ_UNLOCK(&pAd->BulkOutLock[0], irqFlag);
			RTUSBEnqueueInternalCmd(pAd, CMDTHREAD_RESET_BULK_OUT, NULL, 0);
		}
		else
