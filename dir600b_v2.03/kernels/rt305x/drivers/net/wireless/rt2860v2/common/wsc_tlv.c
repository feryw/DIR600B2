	...
	while (PlainLength > 4)
	{
		...
		switch (WscType)
		{
			...
			case WSC_ID_MAC_ADDR:
				if (!MAC_ADDR_EQUAL(pData, pAdapter->StaCfg.WscControl.RegData.EnrolleeInfo.MacAddr))
		...
		switch (WscType)
		{
			...
			case WSC_ID_SC_STATE:
				...
				IF_DEV_CONFIG_OPMODE_ON_STA(pAdapter)
				{
					...
					pWscControl->bConfiguredAP = (pReg->EnrolleeInfo.ScState == WSC_SCSTATE_CONFIGURED) ? TRUE:FALSE;
				}
#endif // CONFIG_STA_SUPPORT //
				FieldCheck[(WSC_TLV_BYTE2(WSC_ID_SC_STATE))] ^= (1 << WSC_TLV_BYTE1(WSC_ID_SC_STATE));
				break;
