...
#ifdef RT2870
			// TODO: Shiang, need to remove it when do formal release, it's used for debug.
	...
	IN TX_BLK *pTxBlk)
{
	...
	{
#ifdef CONFIG_STA_SUPPORT
		if ( (pAd->OpMode == OPMODE_STA) && (ADHOC_ON(pAd)) && (pTxBlk->Wcid > BSSID_WCID))
			pTxBlk->pMacEntry = &pAd->MacTab.Content[BSSID_WCID];	            	// TODO: use different entry setting
        else
#endif // CONFIG_STA_SUPPORT //	    
...
#ifdef CONFIG_STA_SUPPORT									
			// When user sets incorrect rate setting,
			// check and follow wireless mode and max supported rate of AP.
    	    if (pTransmit->field.MODE > pAd->StaCfg.HTPhyMode.field.MODE)
	        	pTransmit->field.MODE = pAd->StaCfg.HTPhyMode.field.MODE;
 
	        if (pTransmit->field.MCS > pAd->StaCfg.HTPhyMode.field.MCS)
    		    pTransmit->field.MCS = pAd->StaCfg.HTPhyMode.field.MCS;
#endif // CONFIG_STA_SUPPORT //						
	}
#endif
