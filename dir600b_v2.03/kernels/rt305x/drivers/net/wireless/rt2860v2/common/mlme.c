...
extern union uLedParameter LedParameter;
...
UCHAR	NUM_OF_3022_CHNL = (sizeof(FreqItems3022) / sizeof(FREQUENCY_ITEM));
	...
	IN PVOID SystemSpecific3) 
{
	...
	IF_DEV_CONFIG_OPMODE_ON_STA(pAd)
	{
	    // snowpin 20071231 sync with windows 2860XpPowerSave3 ++
...
//	RECBATimerTimeout(SystemSpecific1,FunctionContext,SystemSpecific2,SystemSpecific3);
	...
	if ((pAd->Mlme.PeriodicRound % 5 == 0) && RTMPAutoRateSwitchCheck(pAd)/*(OPSTATUS_TEST_FLAG(pAd, fOP_STATUS_TX_RATE_SWITCH_ENABLED))*/)
	{
		...
		IF_DEV_CONFIG_OPMODE_ON_STA(pAd)
		{
			...
			if (pAd->StaCfg.BssType == BSS_INFRA)
				MlmeDynamicTxRateSwitchingEx(pAd, 2);
#endif // MESH_SUPPORT //
		}
#endif // CONFIG_STA_SUPPORT //
	}
	...
	//		pAd->RalinkCounters.MgmtRingFullCount = 0;
		...
		IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
		{
			...
			// 6. AP is not in DFS silence mode
...
//
	...
	if (INFRA_ON(pAd))
	{
		...
		if (INFRA_ON(pAd) &&
			(pAd->StaCfg.LastBeaconRxTime + 3.5*HZ < pAd->Mlme.Now32))  // BEACON almost starving?
		{
...
// TODO: temp removed
	...
	else if (ADHOC_ON(pAd))
	{
			...
			&& RadarChannelCheck(pAd, pAd->CommonCfg.Channel))
		{
			...
			{
				pAd->CommonCfg.RadarDetect.RDCount++;
				
				// channel availability check time is 60sec
				if (pAd->CommonCfg.RadarDetect.RDCount > 65)
				{
			...
			OPSTATUS_TEST_FLAG(pAd, fOP_STATUS_MEDIA_STATE_CONNECTED))
		{
			...
			RTMPZeroMemory(&pAd->MacTab, sizeof(MAC_TABLE));
			NdisReleaseSpinLock(&pAd->MacTabLock);
...
//	Rssi = RTMPMaxRssi(pAd, (CHAR)pAd->StaCfg.AvgRssi0, (CHAR)pAd->StaCfg.AvgRssi1, (CHAR)pAd->StaCfg.AvgRssi2);
	...
	if (TxTotalCnt)
		TxErrorRatio = ((pAd->RalinkCounters.OneSecTxRetryOkCount + pAd->RalinkCounters.OneSecTxFailCount) * 100) / TxTotalCnt;
#else
	TxTotalCnt = TxSuccess + TxRetransmit + TxFailCount;

	if (TxTotalCnt)
		TxErrorRatio = ((TxRetransmit + TxFailCount) * 100) / TxTotalCnt;
#endif

	DBGPRINT_RAW(RT_DEBUG_INFO,("DRS: OneSecTxRetryOkCount=%ld, OneSecTxFailCount=%ld, OneSecTxNoRetryOkCount=%ld \n",
			TxRetransmit,
			TxFailCount,
			TxSuccess));
...
/*
	...
	{
		DBGPRINT_RAW(RT_DEBUG_INFO,("DRS: Fixed - CurrTxRateIdx=%d, MCS=%d, STBC=%d, ShortGI=%d, Mode=%d, BW=%d, PER=%ld%% \n\n",
			pAd->CommonCfg.TxRateIndex,	pAd->StaCfg.HTPhyMode.field.MCS,	pAd->StaCfg.HTPhyMode.field.STBC,
			pAd->StaCfg.HTPhyMode.field.ShortGI,	pAd->StaCfg.HTPhyMode.field.MODE, pAd->StaCfg.HTPhyMode.field.BW, TxErrorRatio));
		return;
	}
	else
 */	
...
//		if (TxRateIdx != pAd->CommonCfg.TxRateIndex)
		...
		DBGPRINT_RAW(RT_DEBUG_INFO,("DRS: TxTotalCnt <= 15, switch TxRateIndex as (%d) according to RSSI(%d)\n", pAd->CommonCfg.TxRateIndex, Rssi));
		return;
	}

	if (pAd->DrsCounters.fLastSecAccordingRSSI == TRUE)
	{
		pAd->DrsCounters.fLastSecAccordingRSSI = FALSE;
		pAd->DrsCounters.LastSecTxRateChangeAction = 0;
		DBGPRINT_RAW(RT_DEBUG_INFO,("DRS: MCS is according to RSSI or rate changed last sec, and ignore tuning this sec\n"));
		return;
	}
	...
	IN PNDIS_802_11_VARIABLE_IEs pVIE) 
{
	...
	IF_DEV_CONFIG_OPMODE_ON_STA(pAd)
	{
        ...
        NdisZeroMemory(pBss->WpaIE.IE, MAX_CUSTOM_LEN);
        NdisZeroMemory(pBss->RsnIE.IE, MAX_CUSTOM_LEN);
		...
		while ((Length + 2 + (USHORT)pEid->Len) <= LengthVIE)    
		{
			switch(pEid->Eid)
			{
				case IE_WPA:
					...
					{
                        sprintf(pBss->WpaIE.IE, "wpa_ie=%02x%02x", pEid->Eid, pEid->Len);
                        for (idx = 0; idx < pEid->Len; idx++)
                            sprintf(pBss->WpaIE.IE, "%s%02x", pBss->WpaIE.IE, (unsigned char)pEid->Octet[idx]);
                        pBss->WpaIE.IELen = (pEid->Len*2) + 7; // 2: ID(1 byte), LEN (1 byte), 7: len of "wpa_ie="
					}
					break;
                case IE_RSN:
                    if (NdisEqualMemory(pEid->Octet + 2, RSN_OUI, 3))
					{
                        sprintf(pBss->RsnIE.IE, "rsn_ie=%02x%02x", pEid->Eid, pEid->Len);
                        for (idx = 0; idx < pEid->Len; idx++)
                        {
                            sprintf(pBss->RsnIE.IE, "%s%02x", pBss->RsnIE.IE, (unsigned char)pEid->Octet[idx]);
					}
                        pBss->RsnIE.IELen = (pEid->Len*2) + 7; // 2: ID(1 byte), LEN (1 byte), 7: len of "rsn_ie="
			}
				break;
            }
...
if (RTDebugLevel >= RT_DEBUG_INFO)
{
		...
		printk("Ch: %d - ", pInBss->Channel);
		for (x=0; x<pInBss->SsidLen; x++)
		{              
			printk("%c", pInBss->Ssid[x]);			
		}
		printk("\n");
}
#endif // DBG //
		...
		if ((pInBss->BssType == pAd->StaCfg.BssType) && 
			(SSID_EQUAL(Ssid, SsidLen, pInBss->Ssid, pInBss->SsidLen) || (pInBss->Hidden)))
		{
	...
	IN	BOOLEAN			bScan) 
{
	...
	{
		for (index = 0; index < NUM_OF_3022_CHNL; index++)
		{
			if (Channel == FreqItems3022[index].Channel)
			{
				// Programming channel parameters
				RT30xxWriteRFRegister(pAd, RF_R02, FreqItems3022[index].N);
				RT30xxWriteRFRegister(pAd, RF_R03, FreqItems3022[index].K);

				RT30xxReadRFRegister(pAd, RF_R06, (PUCHAR)&Value);
				Value = (Value & 0xFC) | FreqItems3022[index].R;
				RT30xxWriteRFRegister(pAd, RF_R06, (UCHAR)Value);

				// Set Tx0 Power
				RT30xxReadRFRegister(pAd, RF_R12, (PUCHAR)&Value);
				Value = (Value & 0x60) | TxPwer;
				RT30xxWriteRFRegister(pAd, RF_R12, (UCHAR)Value);

				// Set Tx1 Power
				RT30xxReadRFRegister(pAd, RF_R13, (PUCHAR)&Value);
				Value = (Value & 0x60) | TxPwer2;
				RT30xxWriteRFRegister(pAd, RF_R13, (UCHAR)Value);

				// Set RF offset
				RT30xxReadRFRegister(pAd, RF_R23, (PUCHAR)&Value);
				Value = (Value & 0x80) | pAd->RfFreqOffset;
				RT30xxWriteRFRegister(pAd, RF_R23, (UCHAR)Value);
				...
				// Enable RF tuning
				RT30xxReadRFRegister(pAd, RF_R07, (PUCHAR)&Value);
				Value = Value | 0x1;
				RT30xxWriteRFRegister(pAd, RF_R07, (UCHAR)Value);
		...
		DBGPRINT(RT_DEBUG_TRACE, ("SwitchChannel#%d(RF=%d, Pwr0=%d, Pwr1=%d, %dT), N=0x%08x, K=0x%08x, R=0x%08x\n",
			Channel, 
			pAd->RfIcType, 
			TxPwer,
			TxPwer2,
			pAd->Antenna.field.TxPath,
			FreqItems3022[index].N, 
			FreqItems3022[index].K, 
			FreqItems3022[index].R));
	}
	else
	...
	switch (pAd->RfIcType)
	{
		...
		DBGPRINT(RT_DEBUG_TRACE, ("SwitchChannel#%d(RF=%d, Pwr0=%d, Pwr1=%d, %dT) to , R1=0x%08x, R2=0x%08x, R3=0x%08x, R4=0x%08x\n",
			Channel, 
			pAd->RfIcType, 
			(R3 & 0x00003e00) >> 9,
			(R4 & 0x000007c0) >> 6,
...
//else
	{
		DBGPRINT(RT_DEBUG_TRACE, ("==============AsicSetMcastWC2 %x:%x:%x:%x:%x:%x\n",
			BCASTADDR[0],BCASTADDR[1],BCASTADDR[2],BCASTADDR[3], BCASTADDR[4],BCASTADDR[5]));
		offset = MAC_WCID_BASE + MCAST_WCID * HW_WCID_ENTRY_SIZE;
		Addr4 = (ULONG)(MULTICAST_ADDR[0])		 | 
			(ULONG)(MULTICAST_ADDR[1] << 8)  | 
			(ULONG)(MULTICAST_ADDR[2] << 16) |
			(ULONG)(MULTICAST_ADDR[3] << 24);
		RTMP_IO_WRITE32(pAd, offset, Addr4);
		offset += 4;
		// always one BSSID in STA mode
		Addr4 = (ULONG)(MULTICAST_ADDR[4]) | (ULONG)(MULTICAST_ADDR[5] << 8);
		RTMP_IO_WRITE32(pAd, offset, Addr4);
	}
#endif // CONFIG_STA_SUPPORT //
}
	...
	IN PRTMP_ADAPTER pAd) 
{
	...
	{
#if 0
// Jerry, Sam: for IOT test, to get better performance
			...
			case 0x31:
				break;
                        ...
                        case 0x54:
                                {
                                int Temp;
                                Temp = 2*(Command-0x52)+1;
                                LedParameter.LedParamArray[Temp]    = Arg0;
                                LedParameter.LedParamArray[Temp+1]  = Arg1;
                                }
                                break;
#endif // (AP_LED) || (STA_LED) //
...
// 2880-SW-MCU
			...
			case 0x60:
				if (Arg0 == 0 && Arg1 == 0)
				{
					...
					{
						printk("AsicSendCommandToMcu 0x60 ==> stop detection\n");
...
#ifdef DFS_FCC_BW40_FIX
						if ((pAd->CommonCfg.RadarDetect.RDDurRegion == FCC) && (pAd->CommonCfg.HtCapability.HtCapInfo.ChannelWidth  == BW_40))
						{
							pAd->CommonCfg.McuRadarDetectPeriod = (Arg1 & 0x1f) * 2;
							pAd->CommonCfg.McuRadarPeriod += Arg1 & 0x1f;
						}
						else
						{
			...
			case 0x63:
				...
				else
				{
					...
					{
						pAd->CommonCfg.McuCarrierPeriod = Arg0 << ((Command == 61) ? 0 : 7);
	...
	IN OUT	ADD_HT_INFO_IE			*pAddHtInfo)
{
	...
	if ((pAd->OpMode == OPMODE_STA) && (Wcid == BSSID_WCID))
	{
#if 0 //Dennis TODO		
