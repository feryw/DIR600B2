...
#ifdef RT2870
	{ SHOW_RXBULK_INFO,
	  0, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, "rxbulk" },
	{ SHOW_TXBULK_INFO,
	  0, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, "txbulk" },
#endif // RT2870 //
    { RAIO_OFF,
...
#ifdef MESH_SUPPORT
	{ SHOW_MESH_INFO,
	  0, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, "meshinfo" },
	{ SHOW_NEIGHINFO_INFO,
	  0, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, "neighinfo" },
	{ SHOW_MESH_ROUTE_INFO,
	  0, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, "meshrouteinfo" },
	{ SHOW_MESH_ENTRY_INFO,
	  0, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, "meshentryinfo" },
	{ SHOW_MULPATH_INFO,
	  0, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, "multipathinfo" },
	{ SHOW_MCAST_AGEOUT_INFO,
	  0, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, "mcastageoutinfo" },
	{ SHOW_MESH_PKTSIG_INFO,
	  0, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, "pktsiginfo" },
#endif // MESH_SUPPORT //
...
/* --- sub-ioctls definitions --- */    
	...
	{ WSC_SET_PIN,
	  IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, 0, "wsc_pin" },
	{ WSC_SET_SSID,
...
//	{"ShortSlot",					Set_ShortSlot_Proc},
	...
	{"MeshPortalEnable",			Set_MeshPortalEnable_Proc},
	...
	IN	PVOID			pBuf)
{
    ...
    //always use BSS0=0
	AsicAddSharedKeyEntry(pAd, 0, (UCHAR)KeyIdx, CipherAlg, pAd->SharedKey[BSS0][KeyIdx].Key, NULL, NULL);

	pAd->StaCfg.PortSecured = WPA_802_1X_PORT_SECURED;	//For Test
		   ...
		   struct iw_freq *freq, char *extra)
{
	...
	if (dev->priv_flags & INT_MESH)
		ch = pAdapter->MeshTab.MeshChannel;
	else
#endif // MESH_SUPPORT //
                        ...
                        char rssi)
{
    ...
    iq->level = (__u8)(rssi - pAdapter->BbpRssiToDbmDelta);
			...
			struct iw_point *data, char *extra)
{
	...
	for (i = 0; i < pAdapter->ScanTab.BssNr; i++) 
	{
        ...
        if ((pAdapter->ScanTab.BssEntry[i].WpaIE.IELen > 0) &&
            (pAdapter->ScanTab.BssEntry[i].WpaIE.IELen <= MAX_CUSTOM_LEN))
        {
            //UCHAR   idx;
            ...
            NdisMoveMemory(custom, pAdapter->ScanTab.BssEntry[i].WpaIE.IE, iwe.u.data.length);
            previous_ev = current_ev;
        ...
        if ((pAdapter->ScanTab.BssEntry[i].RsnIE.IELen > 0) &&
            (pAdapter->ScanTab.BssEntry[i].RsnIE.IELen <= MAX_CUSTOM_LEN))
        {
            //UCHAR   idx;
            char custom[MAX_CUSTOM_LEN] = {0};
            ...
            NdisMoveMemory(custom, pAdapter->ScanTab.BssEntry[i].RsnIE.IE, iwe.u.data.length);
            previous_ev = current_ev;
    		current_ev = iwe_stream_add_point(current_ev, end_buf, &iwe,  custom);
            if (current_ev == previous_ev)
#if WIRELESS_EXT >= 17
			 ...
			 struct iw_point *data, char *essid)
{
	...
	if (data->flags)
	{
		if (data->length > (IW_ESSID_MAX_SIZE + 1)) //includes null character.
		{
			return -E2BIG;
		}

        memcpy(Ssid.Ssid, essid, data->length);
        if (Ssid.Ssid[data->length - 1] == '\0')
        {
            if (data->length > 0)
		...
		{
		    DBGPRINT(RT_DEBUG_TRACE ,("The length of SSID exceeds 32 !\n"));
			return -E2BIG;
		}
	}
	else    // ANY ssid
	...
	if ((pAdapter->StaCfg.AuthMode >= Ndis802_11AuthModeWPA) && (pAdapter->StaCfg.AuthMode != Ndis802_11AuthModeWPANone))
			 ...
			 struct iw_point *data, char *essid)
{
	...
	if (OPSTATUS_TEST_FLAG(pAdapter, fOP_STATUS_MEDIA_STATE_CONNECTED))
	{
		...
		DBGPRINT(RT_DEBUG_TRACE ,("pAdapter->CommonCfg.Ssid=%s , Ssidlen = %d\n",pAdapter->CommonCfg.Ssid, pAdapter->CommonCfg.SsidLen));
	}
#ifdef RT2870    
#ifdef WPA_SUPPLICANT_SUPPORT
    // Add for RT2870
			 ...
			 u32 *uwrq, char *extra)
{
    ...
    int  Status=0;
    u32 subcmd = *uwrq;
    PWSC_PROFILE    pWscProfile = &pAdapter->StaCfg.WscControl.WscProfile;
   	u32 value = 0;
    ...
    switch(subcmd)
    {
        ...
        case WSC_SET_DRIVER_CONNECT_BY_CREDENTIAL_IDX:
            ...
            {
                WscWriteConfToPortCfg(pAdapter, NULL, &pAdapter->StaCfg.WscControl.WscProfile.Profile[value], TRUE);
...
{    
    ...
    switch(subcmd)
    {
        ...
        case WSC_SET_SSID:
            ...
            if (dataLen > 0 && dataLen <= NDIS_802_11_LENGTH_SSID)
            {
                pAdapter->StaCfg.WscControl.WscSsid.SsidLength = dataLen - 1; // dataLen includes size of '\0'
                NdisZeroMemory(pAdapter->StaCfg.WscControl.WscSsid.Ssid, NDIS_802_11_LENGTH_SSID);
                NdisMoveMemory(pAdapter->StaCfg.WscControl.WscSsid.Ssid, extra, dataLen - 1);
            }
            else
                Status = -E2BIG;
            break;
    ...
    for (idx = 0; idx < pAd->StaCfg.WscControl.WscProfile.ProfileCnt ; idx++)
    {
        ...
        if (pCredential->KeyLength != 0)
        {
                     ...
                     (pCredential->AuthType == WSC_AUTHTYPE_SHARED))
            {
                ...
                if (RTMPCheckStrPrintAble(pCredential->Key, pCredential->KeyLength))
                    sprintf(extra+strlen(extra), "Key                             = %s\n", pCredential->Key);
                else
                {
                    int idx;
    ...
    switch(cmd & 0x7FFF) {
        case RT_OID_802_11_COUNTRY_REGION:
            if (wrq->u.data.length < sizeof(UCHAR))
                Status = -EINVAL;
            else if (!(pAdapter->CommonCfg.CountryRegion & 0x80) && !(pAdapter->CommonCfg.CountryRegionForABand & 0x80))	// Only avaliable when EEPROM not programming
            {
                ULONG   Country;	
                UCHAR	TmpPhy;
...
//                pAdapter->CommonCfg.EnableTurboRate = 0;//StaConfig.EnableTurboRate;
...
//                                        pAdapter->CommonCfg.EnableTurboRate,
        ...
        case OID_802_11_WEP_STATUS:
            ...
            else
            {
                ...
                if (WepStatus <= Ndis802_11Encryption3KeyAbsent)
                {
                    ...
                    //RTMPMakeRSNIE(pAdapter, pAdapter->StaCfg.AuthMode, pAdapter->StaCfg.WepStatus, BSS0);
                }
                else
                {
                    Status  = -EINVAL;
        ...
        case OID_802_11_INFRASTRUCTURE_MODE:
            ...
            else
            {
                ...
                else if (BssType == Ndis802_11Monitor)
                {
    				...
    				if (pAdapter->CommonCfg.RegTransmitSetting.field.BW == BW_40 &&
                        pAdapter->CommonCfg.RegTransmitSetting.field.EXTCHA == EXTCHA_ABOVE)
    				{
    					// 40MHz ,control channel at lower
    					RTMP_BBP_IO_READ8_BY_REG_ID(pAdapter, BBP_R4, &bbpValue);
    				...
    				else if (pAdapter->CommonCfg.RegTransmitSetting.field.BW == BW_40 &&
                             pAdapter->CommonCfg.RegTransmitSetting.field.EXTCHA == EXTCHA_BELOW)
    				{
    					// 40MHz ,control channel at upper
    					RTMP_BBP_IO_READ8_BY_REG_ID(pAdapter, BBP_R4, &bbpValue);
			        ...
			        pAdapter->StaCfg.BssType = BSS_MONITOR;
                    pAdapter->net_dev->type = ARPHRD_IEEE80211; // IEEE80211
			        DBGPRINT(RT_DEBUG_TRACE, ("===>rt_ioctl_siwmode::SIOCSIWMODE (MONITOR)\n"));
                }
                else
                {
            ...
            else
            {
                ...
                if (PowerMode == Ndis802_11PowerModeCAM) 
                {
                    ...
                    if (pAdapter->StaCfg.bWindowsACCAMEnable == FALSE)
                        pAdapter->StaCfg.WindowsPowerMode = PowerMode;
                    pAdapter->StaCfg.WindowsBatteryPowerMode = PowerMode;
                }
        ...
        case OID_802_11_TX_POWER_LEVEL:
            ...
            else
            {
                ...
                if (TxPowerLevel > MAX_TX_POWER_LEVEL)
                    ...
                    pAdapter->CommonCfg.TxPower = (UCHAR)TxPowerLevel;
            }
            DBGPRINT(RT_DEBUG_TRACE, ("Set::OID_802_11_TX_POWER_LEVEL (=%ld) \n",TxPowerLevel));
            break;
...
//				if (pAdapter->PortCfg.TxPowerDefault < pAdapter->PortCfg.TxPowerPercentage)
		...
		case OID_802_11_RX_ANTENNA_SELECTED:  
			...
			{  
				...
				pAdapter->CommonCfg.BandState = UNKNOWN_BAND;
				DBGPRINT(RT_DEBUG_TRACE, ("Set::OID_802_11_RX_ANTENNA_SELECTED (=%d)\n", AntDiv));
				AsicAntennaSelect(pAdapter, pAdapter->LatchRfRegs.Channel);	
			}
			break;
        ...
        case RT_OID_802_11_ADD_WPA:
            ...
            else
            {
				...
				{		
                    //pAdapter->StaCfg.PskKey.KeyLen = (UCHAR) pKey->KeyLength;
                ...
                {   
                    //NdisMoveMemory(pKey->BSSID, pAdapter->CommonCfg.Bssid, 6);
...
//				BA_REC_ENTRY	*pBAEntry;
        ...
        case OID_802_11_DEAUTHENTICATION:
            ...
            else
            {
                ...
                MlmeDeauthReqAction(pAdapter, &ElemDeauththenticationMsg);
                if (INFRA_ON(pAdapter))
                {
                    LinkDown(pAdapter, FALSE);
		...
		case RT_OID_WSC_EAPMSG:
			{
				...
				else
				{
					...
					//assign the STATE_MACHINE type
					{
						...
						retVal = MlmeEnqueueForWsc(pAdapter, msgHdr->envID, senderID, Machine, msgType, msgLen, pUPnPMsg);
        ...
        case RT_OID_WSC_SET_MODE:
            ...
            else
            {
                ...
                {
                    if (WscMode == 1)
                        Set_WscMode_Proc(pAdapter, "1"); // PIN
                    else if (WscMode == 2)
                        Set_WscMode_Proc(pAdapter, "2"); // PBC
                    else
                    {
        ...
        case RT_OID_WSC_SET_PIN_CODE:
            ...
            else
            {
                ...
                if (Status == 0)
                {
                    if (Set_WscPinCode_Proc(pAdapter, &PinCode[0]) == FALSE)
                        Status = -EINVAL;
                }
            }
            break;
        case RT_OID_WSC_SET_SSID:
            ...
            else
            {
                ...
                Status = copy_from_user(&Ssid, wrq->u.data.pointer, wrq->u.data.length);
                pSsid = &Ssid;
                Set_WscSsid_Proc(pAdapter, pSsid->Ssid);
            }
            break;
        case RT_OID_WSC_SET_CONN_BY_PROFILE_INDEX:
            ...
            else
            {
                ...
                {                    
                    ...
                    WscWriteConfToPortCfg(pAdapter, NULL, &pWscControl->WscProfile.Profile[wsc_profile_index], TRUE);
        ...
        case RT_OID_WSC_DRIVER_AUTO_CONNECT:
            ...
            {
                Status = copy_from_user(&pAdapter->StaCfg.WscControl.WscDriverAutoConnect, wrq->u.data.pointer, wrq->u.data.length);
                DBGPRINT(RT_DEBUG_TRACE, ("RT_OID_WSC_DRIVER_AUTO_CONNECT::WscDriverAutoConnect is %s\n", 
                                            pAdapter->StaCfg.WscControl.WscDriverAutoConnect ? "Enabled":"Disabled"));
            }
            break;
        case RT_OID_WSC_SET_PASSPHRASE:
            ...
            {
                Status = copy_from_user(pAdapter->StaCfg.WscControl.WpaPsk, wrq->u.data.pointer, wrq->u.data.length);
                NdisZeroMemory(pAdapter->StaCfg.WscControl.WpaPsk, 64);
                pAdapter->StaCfg.WscControl.WpaPskLen = wrq->u.data.length;
                DBGPRINT(RT_DEBUG_TRACE, ("RT_OID_WSC_SET_PASSPHRASE::KeyLen(%d)\n", pAdapter->StaCfg.WscControl.WpaPskLen));
            }
            break;
#endif // WSC_STA_SUPPORT //
    ...
    IN  INT                 cmd)
{
    ...
    NDIS_802_11_WEP_STATUS              WepStatus;
    //RT_VERSION_INFO                     *pDriverVersionInfo = NULL;
    NDIS_MEDIA_STATE                    MediaState;
    ULONG                               BssBufSize, ulInfo=0, NetworkTypeList[4], apsd = 0;
    USHORT                              BssLen = 0; // length in iw_point is u16
...
// in ExtRateIE which may be mis-treated as 802.11b AP by ZeroConfig
        ...
        case OID_802_11_SSID:
			...
			//NdisMoveMemory(Ssid.Ssid,	pAdapter->CommonCfg.Ssid, Ssid.SsidLength);
        ...
        case RT_OID_802_11_STA_CONFIG:
            ...
            if (pStaConfig)
            {
                ...
                pStaConfig->EnableTurboRate = 0; //pAdapter->PortCfg.EnableTurboRate;
		...
		case RT_OID_WSC_QUERY_STATUS:
			...
			if (copy_to_user(wrq->u.data.pointer, &pAdapter->StaCfg.WscControl.WscStatus, wrq->u.data.length))
			{
				Status = -EFAULT;
			}
			DBGPRINT(RT_DEBUG_TRACE, ("Query::RT_OID_WSC_QUERY_STATUS (=%d)\n", pAdapter->StaCfg.WscControl.WscStatus));
			break;

		case RT_OID_WSC_PIN_CODE:
			wrq->u.data.length = sizeof(UINT);
			//WscPinCode = WscGeneratePinCode(pAdapter, 0);
			WscPinCode = pAdapter->StaCfg.WscControl.WscEnrolleePinCode;
        ...
        case RT_OID_WSC_QUERY_DEFAULT_PROFILE:
            ...
            WscCreateProfileFromCfg(pAdapter, STA_MODE, &pAdapter->StaCfg.WscControl, pProfile);
		...
		case RT_OID_802_11_WSC_QUERY_PROFILE:
			...
			pProfile = kmalloc(sizeof(WSC_PROFILE), MEM_ALLOC_FLAG);
			RTMPZeroMemory(pProfile, sizeof(WSC_PROFILE));
			NdisMoveMemory(pProfile, &pAdapter->StaCfg.WscControl.WscProfile, sizeof(WSC_PROFILE));
		...
		case RT_OID_WSC_UUID:
			...
			if (copy_to_user(wrq->u.data.pointer, &pAdapter->Wsc_Uuid_Str[0], UUID_LEN_STR))
			{
				Status = -EFAULT;
			}
			break;
	...
	IN	INT					cmd)
{
	...
	{			
		...
		case RT_PRIV_IOCTL:
#ifdef WSC_STA_SUPPORT
        case RT_PRIV_IOCTL_SYNC_AP_FOR_WSC_UPNP:
#endif // WSC_STA_SUPPORT //
...
#ifdef RT2870
            // TODO: Need to fix it
            DBGPRINT(RT_DEBUG_ERROR, ("%s(): NeedToFixMe!\n", __FUNCTION__));
#endif // RT2870 //
    ...
    IN  PUCHAR          arg)
{
    ...
    pAdapter->StaCfg.PortSecured = WPA_802_1X_PORT_NOT_SECURED;
	//RTMPMakeRSNIE(pAdapter, pAdapter->StaCfg.AuthMode, pAdapter->StaCfg.WepStatus, 0);
    ...
    IN  PUCHAR          arg)
{
...
#ifdef WSC_STA_SUPPORT
    NdisZeroMemory(pAdapter->StaCfg.WscControl.WpaPsk, 64);
    pAdapter->StaCfg.WscControl.WpaPskLen = 0;    
    NdisMoveMemory(pAdapter->StaCfg.WscControl.WpaPsk, arg, strlen(arg));
    pAdapter->StaCfg.WscControl.WpaPskLen = (INT)strlen(arg);
#endif // WSC_STA_SUPPORT //
    ...
    {
/*    
		NdisZeroMemory(&pAdapter->SharedKey[BSS0][0], sizeof(CIPHER_KEY));  
		pAdapter->SharedKey[BSS0][0].KeyLen = LEN_TKIP_EK;
		NdisMoveMemory(pAdapter->SharedKey[BSS0][0].Key, pAdapter->StaCfg.PMK, LEN_TKIP_EK);
		NdisMoveMemory(pAdapter->SharedKey[BSS0][0].RxMic, &pAdapter->StaCfg.PMK[16], LEN_TKIP_RXMICK);
		NdisMoveMemory(pAdapter->SharedKey[BSS0][0].TxMic, &pAdapter->StaCfg.PMK[16], LEN_TKIP_TXMICK);            

		// Decide its ChiperAlg
		if (pAdapter->StaCfg.PairCipher == Ndis802_11Encryption2Enabled)
			pAdapter->SharedKey[BSS0][0].CipherAlg = CIPHER_TKIP;
		else if (pAdapter->StaCfg.PairCipher == Ndis802_11Encryption3Enabled)
			pAdapter->SharedKey[BSS0][0].CipherAlg = CIPHER_AES;
		else
			pAdapter->SharedKey[BSS0][0].CipherAlg = CIPHER_NONE;
*/
        pAdapter->StaCfg.WpaState = SS_NOTUSE;     
    }
    else
	...
	IN	PUCHAR			arg)
{
    ...
    if ((WscMode == WSC_PIN_MODE ) || (WscMode == WSC_PBC_MODE))
    {
        ...
        // generate pin code
        pWscControl->WscPinCode = WscGeneratePinCode(pAd,
#ifdef CONFIG_AP_SUPPORT
        											FALSE,
#endif // CONFIG_AP_SUPPORT //
        											0);
...
//    BOOLEAN		        Cancelled;
	...
	if (INFRA_ON(pAd))
	{
		...
		{
			RT28XX_MLME_RESET_STATE_MACHINE(pAd);
			DBGPRINT(RT_DEBUG_TRACE, ("!!! MLME busy, reset MLME state machine !!!\n"));
		}
		...
		RTMPusecDelay(10000);  // leave enough time for this DISASSOC frame
	}
	else if (ADHOC_ON(pAd))
	{
		// Set the AutoReconnectSsid to prevent it reconnect to old SSID
	...
	if (pWscControl->WscMode == 1)
	{
		...
		{
			NdisZeroMemory(&Ssid, sizeof(NDIS_802_11_SSID));
			NdisMoveMemory(Ssid.Ssid, pWscControl->WscSsid.Ssid, pWscControl->WscSsid.SsidLength);
			Ssid.SsidLength = pWscControl->WscSsid.SsidLength;
