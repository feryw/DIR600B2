	...
	IN ULONG			UpdatePos)
{
	...
	UCHAR  			*ptr;
	UINT  			i, padding;
	BEACON_SYNC_STRUCT	*pBeaconSync = pAd->ApCfg.pBeaconSync;
	UINT32			longValue;
//	USHORT			shortValue;
	...
	if (BeaconTransmitRequired(pAd, apidx) == FALSE)
	{
		...
		pBeaconSync->BeaconBitMap &= (~(BEACON_BITMAP_MASK & (1 << apidx)));
	...
	else
	{
...
#endif
		if (NdisEqualMemory(pBeaconSync->BeaconTxWI[apidx], &pAd->BeaconTxWI, TXWI_SIZE) == FALSE)
		{	// If BeaconTxWI changed, we need to rewrite the TxWI for the Beacon frames.
			pBeaconSync->BeaconBitMap &= (~(BEACON_BITMAP_MASK & (1 << apidx)));
			NdisMoveMemory(pBeaconSync->BeaconTxWI[apidx], &pAd->BeaconTxWI, TXWI_SIZE);
		}
		
		if ((pBeaconSync->BeaconBitMap & (1 << apidx)) != (1 << apidx))
		{
		...
		ptr = pBeaconSync->BeaconBuf[apidx];
		...
		for (i = 0 ; i < FrameLen /*HW_BEACON_OFFSET*/; i += 2)
		{
			if (NdisEqualMemory(ptr, pBeaconFrame, 2) == FALSE)
			{
				...
				//RTMP_IO_WRITE8(pAd, pAd->BeaconOffset[apidx] + TXWI_SIZE + i, shortValue);
				RTUSBMultiWrite(pAd, pAd->BeaconOffset[apidx] + TXWI_SIZE + i, ptr, 2);
			}
			ptr +=2;
			pBeaconFrame += 2;
		}

		pBeaconSync->BeaconBitMap |= (1 << apidx);
	}
    ...
    IN PVOID SystemSpecific3)
{
	...
	pAd->ApCfg.BeaconUpdateTimer.TimerValue = (delta >> 10) + 20;
#if 0
	printk("Adj:b=%d%d, a=%d%d, exp=%d%d, dc=%d, d=%d, np=%ld\n", 
			tsfTime_b.HighPart, tsfTime_b.LowPart, tsfTime_a.HighPart, tsfTime_a.LowPart, 
			expectedTime.HighPart, expectedTime.LowPart, pAd->ApCfg.DtimCount, delta, pAd->ApCfg.BeaconUpdateTimer.TimerValue);
#endif	
...
//	BOOLEAN			positive;
	...
	//positive=getDeltaTime(tsfTime_a, expectedTime, &deltaTime_exp);
	remain_high = pAd->ApCfg.BeaconRemain * tsfTime_a.HighPart;
	remain_low = tsfTime_a.LowPart % (pAd->CommonCfg.BeaconPeriod << 10);
	remain = (remain_high + remain_low)%(pAd->CommonCfg.BeaconPeriod << 10);
	delta = (pAd->CommonCfg.BeaconPeriod << 10) - remain;

	pAd->ApCfg.BeaconUpdateTimer.TimerValue = (delta >> 10) + 30;
	...
	IN RTMP_ADAPTER *pAd)
{
	...
	pBeaconSync = pAd->ApCfg.pBeaconSync;
...
//	LARGE_INTEGER 	tsfTime, deltaTime;
	...
	if (pBeaconSync && pBeaconSync->EnableBeacon)
	{
		for(apidx=0; apidx<pAd->ApCfg.BssidNum; apidx++)
		{
			...
			pBeaconSync->CapabilityInfoLocationInBeacon[apidx] = pAd->ApCfg.MBSSID[apidx].CapabilityInfoLocationInBeacon;
			pBeaconSync->TimIELocationInBeacon[apidx] = pAd->ApCfg.MBSSID[apidx].TimIELocationInBeacon;
			pBeaconSync->BeaconBitMap |= (1<<pAd->ApCfg.BssidNum);
			NdisZeroMemory(pBeaconSync->BeaconTxWI[apidx], TXWI_SIZE);
		}

		pAd->ApCfg.BeaconUpdateTimer.Repeat = TRUE;

		pAd->ApCfg.BeaconAdjust = 0;
		pAd->ApCfg.BeaconFactor = 0xffffffff / (pAd->CommonCfg.BeaconPeriod << 10);
		pAd->ApCfg.BeaconRemain = (0xffffffff % (pAd->CommonCfg.BeaconPeriod << 10)) + 1;
		printk("RT2870_BssBeaconStart:BeaconFactor=%d, BeaconRemain=%d!\n", pAd->ApCfg.BeaconFactor, pAd->ApCfg.BeaconRemain);
		RTMPSetTimer(&pAd->ApCfg.BeaconUpdateTimer, pAd->CommonCfg.BeaconPeriod);
	...
	IN RTMP_ADAPTER *pAd)
{
	...
	NdisAllocMemory(pAd->ApCfg.pBeaconSync, sizeof(BEACON_SYNC_STRUCT), MEM_ALLOC_FLAG);
	if (pAd->ApCfg.pBeaconSync)
	{
		...
		//RTMPInitTimer(pAd, &pAd->ApCfg.BeaconUpdateTimer, GET_TIMER_FUNCTION(APBeaconUpdateExec), pAd, TRUE);
	...
	IN RTMP_ADAPTER *pAd)
{
	...
	if (pAd->ApCfg.pBeaconSync)
	{
		...
		NdisFreeMemory(pAd->ApCfg.pBeaconSync, HW_BEACON_OFFSET * HW_BEACON_MAX_COUNT, 0);
		pAd->ApCfg.pBeaconSync = NULL;
	}
	DBGPRINT(RT_DEBUG_INFO,("<---RT2870_BssBeaconExit()!\n"));	
}
