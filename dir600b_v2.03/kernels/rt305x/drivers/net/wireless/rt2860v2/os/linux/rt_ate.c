...
extern UCHAR NUM_OF_3022_CHNL;
extern FREQUENCY_ITEM FreqItems3022[];
...
/*
    ...
    Description:
        ...
        0. APSTOP  = Start ATE Mode / 0. ATESTART  = Start ATE Mode(DPA)
        1. APSTART   = Stop ATE Mode / 1. ATESTOP   = Stop ATE Mode(DPA)
	...
	IN	PUCHAR			arg)
{
	...
	{
		ATEDBGPRINT(RT_DEBUG_TRACE, ("ATE: APSTOP\n"));
#endif
...
//		AsicEnableBssSync(pAd);
	...
	{						
		ATEDBGPRINT(RT_DEBUG_TRACE, ("ATE: APSTART\n"));
#endif
...
//			pHeader80211 = (PHEADER_802_11)pDest;
...
#ifdef CONFIG_AP_SUPPORT
			pAd->NullFrame.FC.FrDs = 1;
			pAd->NullFrame.FC.ToDs = 0;
#endif // CONFIG_AP_SUPPORT //
#ifdef CONFIG_STA_SUPPORT
		...
		RTMPZeroMemory(&pAd->NullContext.TransferBuffer->WirelessPacket[0], TX_BUFFER_NORMSIZE);//???
		...
		if (pAd->ate.bQATxStart == TRUE) 
		{
			...
			ATEWriteTxWI(pAd, pTxWI, pAd->ate.TxWI.FRAG, pAd->ate.TxWI.TS, pAd->ate.TxWI.AMPDU, pAd->ate.TxWI.ACK, pAd->ate.TxWI.NSEQ, 
				pAd->ate.TxWI.BAWinSize, BSSID_WCID, pAd->ate.TxWI.MPDUtotalByteCount/* include 802.11 header */, pAd->ate.TxWI.PacketId, MMPS_STATIC, pAd->ate.TxWI.txop/*IFS_HTTXOP*/, pAd->ate.TxWI.CFACK/*FALSE*/, TxHTPhyMode);
		}
		else
		{
			TxHTPhyMode.field.BW = pAd->ate.TxWI.BW;
			...
			ATEWriteTxWI(pAd, pTxWI,  FALSE, FALSE, FALSE, FALSE/* No ack required. */, FALSE, 0, BSSID_WCID, pAd->ate.TxLength,
				0, MMPS_STATIC, IFS_HTTXOP, FALSE, TxHTPhyMode);// "MMPS_STATIC" instead of "MMPS_DYNAMIC" ???
		}

		RTMPMoveMemory(&pAd->NullContext.TransferBuffer->WirelessPacket[TXINFO_SIZE+TXWI_SIZE], &pAd->NullFrame, sizeof(HEADER_802_11));

		pDest = &(pAd->NullContext.TransferBuffer->WirelessPacket[TXINFO_SIZE+TXWI_SIZE+sizeof(HEADER_802_11)]);
...
VOID ATE_RTUSBBulkOutDataPacketComplete(purbb_t pUrb, struct pt_regs *pt_regs)
{
	...
	ATEDBGPRINT(RT_DEBUG_INFO, ("--->ATE_RTUSBBulkOutDataPacketComplete\n"));
	...
	{	
		...
		if ((pAd->ate.Mode != ATE_STOP) && (pAd->ate.bQATxStart == TRUE))
		{
			if (pAd->ate.QID == BulkOutPipeId)
			{
				...
				RTMPusecDelay(500);
				pAd->ate.TxDoneCount++;
				ATEDBGPRINT(RT_DEBUG_INFO, ("pAd->ate.TxDoneCount == %d\n", pAd->ate.TxDoneCount));
...
*/
	...
	ATEDBGPRINT(RT_DEBUG_LOUD, ("pNullContext->pAd = 0x%lx\n", (ULONG)&pNullContext->pAd));
	ATEDBGPRINT(RT_DEBUG_LOUD, ("pNullContext->pUrb = 0x%lx\n", (ULONG)&pNullContext->pUrb));
	ATEDBGPRINT(RT_DEBUG_LOUD, ("pNullContext->TransferBuffer = 0x%lx\n", (ULONG)&pNullContext->TransferBuffer));
	ATEDBGPRINT(RT_DEBUG_LOUD, ("pNullContext->BulkOutPipeId = %d\n", pNullContext->BulkOutPipeId));
	ATEDBGPRINT(RT_DEBUG_LOUD, ("pNullContext->BulkOutSize = %ld\n", pNullContext->BulkOutSize));
	ATEDBGPRINT(RT_DEBUG_LOUD, ("pNullContext->InUse = %d\n", (pNullContext->InUse==TRUE)));
	ATEDBGPRINT(RT_DEBUG_LOUD, ("pNullContext->bWaitingBulkOut = %d\n", (pNullContext->bWaitingBulkOut==TRUE)));
	ATEDBGPRINT(RT_DEBUG_LOUD, ("pNullContext->IRPPending = %d\n", (pNullContext->IRPPending==TRUE)));
	ATEDBGPRINT(RT_DEBUG_LOUD, ("pNullContext->LastOne = %d\n", (pNullContext->LastOne==TRUE)));

	if (atomic_read(&pAd->BulkOutRemained) > 0)
	{			
		atomic_dec(&pAd->BulkOutRemained);
		ATEDBGPRINT(RT_DEBUG_INFO, ("Bulk Out Remained = %d\n", atomic_read(&pAd->BulkOutRemained)));
	}
	
	// 1st - Transmit Success
	OldValue = pAd->WlanCounters.TransmittedFragmentCount.LowPart;
	pAd->WlanCounters.TransmittedFragmentCount.LowPart++;

	if (pAd->WlanCounters.TransmittedFragmentCount.LowPart < OldValue)
	{
		pAd->WlanCounters.TransmittedFragmentCount.HighPart++;
	}
	
	if(((pAd->ContinBulkOut == TRUE ) ||(atomic_read(&pAd->BulkOutRemained) > 0)) && (pAd->ate.Mode & ATE_TXFRAME))
	{	
		ATEDBGPRINT(RT_DEBUG_INFO, ("Continue to BulkOut ! \n"));
		RTUSB_SET_BULK_FLAG(pAd, fRTUSB_BULK_OUT_DATA_ATE);
	}	
	else
	{
	...
	IN	UCHAR			BulkOutPipeId)
{
	...
	ATEDBGPRINT(RT_DEBUG_INFO, ("--->ATE_RTUSBBulkOutDataPacket \n"));
...
//	ATESetUpFrame(pAd, 0);
	...
	pNullContext->IRPPending = TRUE;
	RTUSBInitTxDesc(pAd, pNullContext, BulkOutPipeId, ATE_RTUSBBulkOutDataPacketComplete);
	...
	IN	PRTMP_ADAPTER	pAd)
{
	...
	ATEDBGPRINT(RT_DEBUG_OFF, ("--->ATE_RTUSBCancelPendingBulkInIRP\n"));
#if 1
	for ( i = 0; i < (RX_RING_SIZE); i++)
	{
		...
		if (pRxContext->IRPPending == TRUE)
		{
			...
			// Sleep 200 microsecs to give cancellation time to work
			NdisMSleep(200);// busy-wait loop : CPU cannot do anything else now.

			/* pAd->BulkInReq is just for statistics(no matter bulkin is fail or not) */
			/*
			if (pAd->BulkInReq > 0)
			{
				pAd->BulkInReq--;
	...
	IN	struct iwreq	*wrq)
{
	...
	switch (Command_Id) 
	{
		...
		case RACFG_CMD_ATE_E2PROM_WRITE_BULK:
		{
			...
			rt_ee_read_all(pAdapter,(USHORT *)buffer);
			memcpy_exs(pAdapter, (UCHAR *)buffer, (UCHAR *)pRaCfg->data + 2 + offset, len);
			rt_ee_write_all(pAdapter,(USHORT *)buffer);
		...
		case RACFG_CMD_ATE_IO_WRITE_BULK:
		{
			...
			for (i = 0; i < len; i += 4)
			{
				memcpy_exl(pAdapter, &value, pRaCfg->data+4+i, 4);
				printk("Write %x %x\n", offset + i, value);
