...
/*
	==========================================================================
	Description: Ssid operation for WSC mode
	...
	IN MLME_QUEUE_ELEM * Elem) 
{
	...
	// protect against invalid length
	if (pOidSsid->SsidLength > MAX_LEN_OF_SSID)
		return;
	...
	DBGPRINT(RT_DEBUG_TRACE, ("CntlWscSsidProc - %d BSS of %d BSS match the desire SSID - %s\n",
			pAd->MlmeAux.SsidBssTab.BssNr, pAd->ScanTab.BssNr, pAd->MlmeAux.Ssid));

	Now = jiffies;
	// There shall be only 1 AP matched the desired AP
	// If connection is up, drop the connection immediately
	if (OPSTATUS_TEST_FLAG(pAd, fOP_STATUS_MEDIA_STATE_CONNECTED))
	{
		DBGPRINT(RT_DEBUG_TRACE, ("CntlWscSsidProc - disassociate with current AP...\n"));
		DisassocParmFill(pAd, &DisassocReq, pAd->CommonCfg.Bssid, REASON_DISASSOC_STA_LEAVING);
		MlmeEnqueue(pAd, ASSOC_STATE_MACHINE, MT2_MLME_DISASSOC_REQ, 
					sizeof(MLME_DISASSOC_REQ_STRUCT), &DisassocReq);
		pAd->Mlme.CntlMachine.CurrState = CNTL_WAIT_DISASSOC;
	...
	{
		// Only infrastructure mode, no Adhoc mode
		// No AP avaliable, this will only happened in PIN mode
		// TODO: We might need to add another scan here
		// TODO: For PBC mode, we might do rescan if no AP found
		if (pAd->MlmeAux.SsidBssTab.BssNr == 0)
		{
			DBGPRINT(RT_DEBUG_TRACE, ("CntlWscSsidProc - No matching BSS\n"));
			pAd->Mlme.CntlMachine.CurrState = CNTL_IDLE;
		}
		else
		{
			// 1 AP ready for association, and the index will always be 0
			CntlWscIterate(pAd);
		}
	}
}
	...
	IN MLME_QUEUE_ELEM *Elem) 
{
	...
	if (Elem->MsgType == MT2_DISASSOC_CONF) 
	{
		...
		else
		{
			...
			if (pAd->StaCfg.WscControl.WscState == WSC_STATE_START)
				CntlWscIterate(pAd);
			else if ((pAd->StaCfg.WscControl.bWscTrigger == FALSE) &&(pAd->StaCfg.WscControl.WscState != WSC_STATE_INIT))
#endif // WSC_STA_SUPPORT //
			IterateOnBssTab(pAd);
		}
	}
	...
	IN UCHAR BssType) 
{
	...
	UINT32	Data;
	BOOLEAN	Cancelled;
	//TX_STA_FIFO_STRUC NewTxRxCsr4, CurTxRxCsr4;
	UCHAR	Value = 0, idx;// Value;
		...
		if (MESH_ON(pAd))
		{
			...
			if ((pAd->CommonCfg.Channel != pAd->MeshTab.MeshChannel))
			{
				if (pAd->MeshTab.UCGEnable == TRUE)
				{
					...
					MlmeEnqueueEx(pAd, MESH_CTRL_STATE_MACHINE, MESH_CTRL_UCG_EVT, 0, NULL, 0);
				}
				else
					pAd->MeshTab.MeshChannel = pAd->CommonCfg.Channel;
			}
...
#if 0
		AsicSetBssidWC(pAd, pAd->CommonCfg.Bssid);
#else
...
#if 0
		// If WEP is used, set its WCID attribute table.		 
		if (pAd->StaCfg.WepStatus == Ndis802_11WEPEnabled)
		{
		
			UCHAR KeyIdx		= pAd->StaCfg.DefaultKeyId;
			UCHAR CipherAlg 	= pAd->SharedKey[BSS0][KeyIdx].CipherAlg;
	...
	if (pAd->StaCfg.WscControl.WscConfMode != WSC_DISABLE && pAd->StaCfg.WscControl.bWscTrigger)
	{
		...
		WscSendEapolStart(pAd, pAd->CommonCfg.Bssid, FALSE);
	}
    else
        WscStop(pAd,
	...
	IN  BOOLEAN      IsReqFromAP)
{
	...
	else					// Infra structure mode
	{
		...
		BATableTearORIEntry(pAd, 0, BSSID_WCID, TRUE);
#else
...
//#ifdef RT2860
	...
	if (pAd->ate.Mode != ATE_STOP)
	{
		...
		{
			pAd->CommonCfg.BBPCurrentBW = BW_20;
			ATE_BBP_IO_READ8_BY_REG_ID(pAd, BBP_R4, &ByteValue);
			ByteValue &= (~0x18);
			ATE_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R4, ByteValue);
		}

		// Reset DAC
		ATE_BBP_IO_READ8_BY_REG_ID(pAd, BBP_R1, &ByteValue);
		ByteValue &= (~0x18);
		if (pAd->Antenna.field.TxPath == 2)
		{
			ByteValue |= 0x10;
		}
		ATE_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R1, ByteValue);
	}
	else
#endif // RALINK_ATE //
//#endif // RT2860 //
    ...
    /*{
		...
		IntCfg.field.TXDLY_INT_EN = 1;
		IntCfg.field.TXMAX_PINT = 0xa;	// unit:packets (not descriptors)
		IntCfg.field.TXMAX_PTIME = 0x28;	//800us.  unit = 20us
		IntCfg.field.RXDLY_INT_EN = 1;
		IntCfg.field.RXMAX_PINT = 0x7;
		IntCfg.field.RXMAX_PTIME = 0x10;
	...
	IN PRTMP_ADAPTER pAd)
{
	...
	RTMPZeroMemory(&pAd->PsPollContext.TransferBuffer->WirelessPacket[0], 100);
	pTxInfo = (PTXINFO_STRUC)&pAd->PsPollContext.TransferBuffer->WirelessPacket[0];
	RTMPWriteTxInfo(pAd, pTxInfo, (USHORT)(sizeof(PSPOLL_FRAME)+TXWI_SIZE), TRUE, EpToQueue[MGMTPIPEIDX], FALSE,  FALSE);
	pTxWI = (PTXWI_STRUC)&pAd->PsPollContext.TransferBuffer->WirelessPacket[TXINFO_SIZE];
#if 0
	RTMPWriteTxWI(pAd, pTxWI, FALSE, FALSE, FALSE, TRUE, FALSE, 0, BSSID_WCID, (sizeof(PSPOLL_FRAME)),
		0, 0,  IFS_HTTXOP, FALSE ,pAd->CommonCfg.MlmeTransmit);
#else
...
#endif
	RTMPMoveMemory(&pAd->PsPollContext.TransferBuffer->WirelessPacket[TXWI_SIZE+TXINFO_SIZE], &pAd->PsPollFrame, sizeof(PSPOLL_FRAME));
...
#if 0
	RTMPWriteTxWI(pAd, pTxWI, FALSE, FALSE, FALSE, TRUE, FALSE, 0, BSSID_WCID, (sizeof(HEADER_802_11)),
		0, 0,  IFS_HTTXOP, FALSE ,pAd->CommonCfg.MlmeTransmit);
#else
