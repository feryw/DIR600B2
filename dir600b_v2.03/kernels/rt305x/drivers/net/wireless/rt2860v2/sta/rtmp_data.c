...
#endif // MESH_SUPPORT //
	{
		...
		if (pRxD->MyBss == 0)
		{
				...
				(pRxD->Decrypted == 0))
			{
				...
				if (pHeader->FC.Wep == 1)
				{
					if (RTMPSoftDecryptBroadCastData(pAd, pRxBlk, pAd->StaCfg.GroupCipher, pAd->SharedKey[BSS0]) == NDIS_STATUS_FAILURE)
					{
						// release packet
						RELEASE_NDIS_PACKET(pAd, pRxPacket, NDIS_STATUS_FAILURE);
						return;
	...
	for (Index = 0; Index < NumberOfPackets; Index++)
	{
		...
		do
		{
			...
			if (RTMP_GET_PACKET_NET_DEVICE(pPacket) >= MIN_NET_DEVICE_FOR_MESH)
			{
				...
				RTMP_SET_PACKET_WCID(pPacket, wcid); // this field is useless when in STA mode
				break;
			}
#endif // MESH_SUPPORT //
...
//	UCHAR			RTSRequired;
	...
	if (VALID_WCID(Wcid) &&
		(pAd->MacTab.Content[Wcid].ValidAsMesh == TRUE))
	{
		bMeshPkt = TRUE;
		...
		{
            DBGPRINT(RT_DEBUG_INFO, ("I/F(mesh%d) STASendPacket --> Drop packet before mesh secured !!!\n", pEntry->MatchMeshTabIdx)); 
            RELEASE_NDIS_PACKET(pAd, pPacket, NDIS_STATUS_FAILURE); 
            return (NDIS_STATUS_FAILURE);
        }
	}
        ...
        INFRA_ON(pAd))
	{
		...
		if (((pMacEntry->TXBAbitmap & (1<<UserPriority)) == 0) && 
            ((pMacEntry->BADeclineBitmap & (1<<UserPriority)) == 0) &&
            (pAd->StaCfg.PortSecured == WPA_802_1X_PORT_SECURED)
			 // For IOT compatibility, if  
			 // 1. It is Ralink chip or 			  
			 // 2. It is OPEN or AES mode, 
			 // then BA session can be bulit.			 
			 && ((pAd->MlmeAux.APRalinkIe != 0x0) || 
			 	 (pAd->StaCfg.WepStatus == Ndis802_11WEPDisabled || pAd->StaCfg.WepStatus == Ndis802_11Encryption3Enabled))
			)
		{
			BAOriSessionSetUp(pAd, pMacEntry, 0, 0, 10, FALSE);
		}
	}
	...
	IN  TX_BLK          *pTxBlk)
{
	...
	if (OPSTATUS_TEST_FLAG(pAd, fOP_STATUS_WMM_INUSED))
	{
		if ((pAd->MacTab.Content[BSSID_WCID].HTCapability.ExtHtCapInfo.PlusHTC == TRUE) && 
			(pAd->MacTab.Content[BSSID_WCID].HTPhyMode.field.MODE >= MODE_HTMIX) && 
			(pTxBlk->bDHCPFrame == FALSE))
...
#if 0	// remove to STABuildCommon802_11Header
		// Increase Sequence
		pTxBlk->pMacEntry->TxSeq[pTxBlk->UserPriority] = (pTxBlk->pMacEntry->TxSeq[pTxBlk->UserPriority]+1) & MAXSEQ;
#endif
		//
...
#if 0
		//FirstTx = HAL_WriteTxResource(pAd, pTxBlk, FALSE, &FreeNumber);
		//LastTxIdx = FirstTx;
#else
		if (frameNum ==0)
...
#if 0 // For station, didn't need it
...
//	printk("Exit of STA_Legacy_Frame_Tx()!\n");
}
...
//	PACKET_INFO		PacketInfo;
...
#if 0	// Move to fucntion "STASendPacket()"
	//
	// WPA 802.1x secured port control - drop all non-802.1x frame before port secured
	//
	if (((pAd->StaCfg.AuthMode == Ndis802_11AuthModeWPA) || 
	...
	{    
		DBGPRINT(RT_DEBUG_TRACE,("STAHardTransmit --> Drop packet before port secured !!!\n"));
		RELEASE_FRAMES_OF_TXBLK(pAd, pTxBlk, pQEntry, NDIS_STATUS_FAILURE);
		return (NDIS_STATUS_FAILURE);
	}
#endif
	...
	IN	PHEADER_802_11	pHeader)
{
	...
	else if (pRxD->Mcast)
	{
		if (!RX_FILTER_TEST_FLAG(pAd, fRX_FILTER_ACCEPT_ALL_MULTICAST))
		{
			...
			{
				DBGPRINT_RAW(RT_DEBUG_TRACE,("Rx MCAST dropped by RX_FILTER\n"));
				return(NDIS_STATUS_FAILURE);
			}
			else
			{
				if (pHeader == NULL)
				{
					DBGPRINT_RAW(RT_DEBUG_TRACE,("Rx  MCAST pHeader == NULL\n"));
					return(NDIS_STATUS_SUCCESS);
				}
				...
				{
					DBGPRINT(RT_DEBUG_TRACE,("Rx MCAST %02x:%02x:%02x:%02x:%02x:%02x dropped by RX_FILTER\n",
				...
				{
					DBGPRINT(RT_DEBUG_TRACE,("Accept multicast %02x:%02x:%02x:%02x:%02x:%02x\n",
						pHeader->Addr1[0], pHeader->Addr1[1], pHeader->Addr1[2], 
						pHeader->Addr1[3], pHeader->Addr1[4], pHeader->Addr1[5]));
				}*/
			}
	...
	{
		DBGPRINT_RAW(RT_DEBUG_TRACE,("Not U2M, not Mcast, not BcastL\n"));
		return(NDIS_STATUS_FAILURE);
	}
	return(NDIS_STATUS_SUCCESS);	
}
...
// This portion already move to rt_os.c
// RTMPCloneNdisPacket,
