...
#include "ra_qos.h"
#endif
#ifdef CONFIG_RAETH_v2_POWER_SAVING
#include "ra_rfrw.h" /*Added for Link Down Power Saving */
#endif
...
#endif
/*Alpha patch*/
#if defined (CONFIG_RALINK_RT3052) && (CONFIG_RT_3052_ESW_PORT_STATUS_INT)
...
/*Alpha patch +++*/
#if defined (CONFIG_RT_3052_ESW_PORT_STATUS_INT)
...
int __init rather_probe(struct net_device *dev)
{
	...
	dev->base_addr = RA2882ETH_BASE;
		...
		/* Alpha patch - read the current MAC. */
		ra2880_read_mac(zero);
		memcpy(addr.sa_data, zero, 6);
		/* Alpha patch - we don't have MAC address there, use the current MAC. */
#if 0	
...
#ifdef CONFIG_RAETH_v2_POWER_SAVING
        /*Added for Link Down Power Saving */
        rw_rf_reg(0, 0, &my_val);
...
int __init ra2882eth_init(void)
{
	...
	/* Alpha patch - read the default MAC address, which is setup at bootcode. */
