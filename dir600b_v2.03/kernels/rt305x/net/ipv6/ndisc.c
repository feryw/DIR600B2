...
static const char *in4_ntop(const u_char *src, char *dst, size_t size)
{
...
static const char *in6_ntop(const u_char *src, char *dst, size_t size)
{
	/*
	...
	for (i = 0; i < 8; i++) {
		...
		if (i == 6 && best.base == 0 &&
			...
			if (!in4_ntop(src+12, tp, sizeof tmp - (tp - tmp)))
				return (NULL);
			tp += strlen(tp);
			break;
		}
...
#if defined(ELBOX_USE_IPV6)
	//printk("-RA:\n");
	in6_ntop((u_char *)&skb->nh.ipv6h->saddr, in6_dev->cnf.ra_saddr, sizeof(in6_dev->cnf.ra_saddr));
	if (ndisc_parse_options(opt, optlen, &ndopts)) {
		if (in6_dev->cnf.accept_ra_pinfo && ndopts.nd_opts_pi) {
			...
			for (p = ndopts.nd_opts_pi;
				...
				if (((p->nd_opt_len) << 3) < sizeof(struct prefix_info)) continue;
				pinfo = (struct prefix_info *)p;
				in6_ntop((u_char *)&pinfo->prefix.s6_addr16, in6_dev->cnf.ra_prefix, sizeof(in6_dev->cnf.ra_prefix));
				in6_dev->cnf.ra_prefix_len = pinfo->prefix_len;
			}
		}
		in6_dev->cnf.ra_mflag = ra_msg->icmph.icmp6_addrconf_managed;
		in6_dev->cnf.ra_oflag = ra_msg->icmph.icmp6_addrconf_other;
	}
#endif
