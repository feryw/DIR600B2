	      ...
	      int scope, u32 flags)
{
	...
	if (idev->cnf.disable_ipv6 || ipv6_devconf.disable_ipv6) {
		err = -EACCES;
		goto out2;
	}
...
static struct addrconf_sysctl_table
{
	...
	.addrconf_vars = {
		...
		{
			.ctl_name     = CTL_UNNUMBERED,
			.procname     = "ra_prefix_len",
			.data         = &ipv6_devconf.ra_prefix_len,
			.maxlen       = sizeof(int),
			.mode         = 0444,
			.proc_handler = &proc_dointvec,
		},
