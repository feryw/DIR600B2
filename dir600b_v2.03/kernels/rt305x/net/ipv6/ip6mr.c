...
static int pim6_rcv(struct sk_buff **pskb)
{
       ...
       encap = (struct ipv6hdr *)(skb->h.raw +
                                  sizeof(*pim));
...
static int ip6mr_cache_report(struct sk_buff *pkt, vifi_t vifi, int assert)
{
   ...
   if (assert == MRT6MSG_WHOLEPKT)
		skb = skb_realloc_headroom(pkt, -(pkt->nh.raw - pkt->data)
										+sizeof(*msg));
   else
#endif
	...
	if (assert == MRT6MSG_WHOLEPKT) {
		...
		*/
		skb_push(skb, -(pkt->nh.raw - pkt->data));
	 ...
	 */
	/**********************************************************************************************
	 * revised below commented code, caused of 2.6.21 doesn't support 'skb_reset_network_header()'
	 * and 'skb_copy_to_linear_data()'.
	 **********************************************************************************************
	 ...
	 */
	/**********************************************************************************************
	 * revised below commented code, caused of 2.6.21 doesn't support 'skb_reset_transport_header()'
