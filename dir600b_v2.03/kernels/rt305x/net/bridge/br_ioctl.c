...
static int old_dev_ioctl(struct net_device *dev, struct ifreq *rq, int cmd)
{
	...
	case BRCTL_GET_FDB_ENTRIES:
        ...
        case 140:
        {
            ...
            strcpy(br->ionlif, (unsigned char*)args[1]);
            #if 0
            printk("br->ionlif: %s\n", br->ionlif);
            #endif /*0*/
            return 0;
        ...
        case 141:
        {
            unsigned int    *tmp = (unsigned int*)args[1];
            int i;
            br->ipnl_ipaddr     = tmp[0];
            br->ipnl_mask       = tmp[1];
            for (i=0;i<8;i++){
                br->ionl_allowed[i] = tmp[i+2];
            }
            #if 0
            printk("br->ipnl_ipaddr: %x\n", br->ipnl_ipaddr);
            printk("br->ipnl_mask: %x\n", br->ipnl_mask);
            printk("br->ionl_allowed[0]: %x\n", br->ionl_allowed[0]);
            printk("br->ionl_allowed[1]: %x\n", br->ionl_allowed[1]);
            printk("br->ionl_allowed[2]: %x\n", br->ionl_allowed[2]);
            printk("br->ionl_allowed[3]: %x\n", br->ionl_allowed[3]);
            printk("br->ionl_allowed[4]: %x\n", br->ionl_allowed[4]);
            printk("br->ionl_allowed[5]: %x\n", br->ionl_allowed[5]);
            printk("br->ionl_allowed[6]: %x\n", br->ionl_allowed[6]);
            printk("br->ionl_allowed[7]: %x\n", br->ionl_allowed[7]);
            #endif /*0*/
            return 0;
        }
        ...
        case 142:
            if ((unsigned int)args[1]==0){
                br->ionl_sw = IONL_DIS;
                #if 0
                printk("br->ionl_sw: IONL_DIS\n");
                #endif /*0*/
                return 0;
            }else if ((unsigned int)args[1]==1){
                br->ionl_sw = IONL_ENA;
                #if 0
                printk("br->ionl_sw: IONL_ENA\n");
                #endif /*0*/
                return 0;
            }
...
/*+++, MAC Address Translator, Builder, 2008/12/10, start*/
	...
	case 144:
		/*Set MAC Clone Type*/
		br->mat_mode = args[1];
		if (br->mat_mode==MACCLONE_DIS){
			memcpy(br->wl_curr_mac, br->dev->dev_addr, ETH_ALEN);
		}else if(br->mat_mode==MACCLONE_AUTO){
			memcpy(br->wl_curr_mac, br->auto_clone_mac, ETH_ALEN);
		}else if(br->mat_mode==MAC_CLONE_MANUAL){
			memcpy(br->wl_curr_mac, br->manual_clone_mac, ETH_ALEN);
		}
		return 0;

	case 145:
		/*Get MAC Clone Type*/
		if (copy_to_user((void __user *)args[1], &br->mat_mode, sizeof(br->mat_mode)));
			return -EFAULT;
		return 0;
	...
	case 146:
	{
		...
		memcpy(macaddr, (unsigned char*)args[1], 6);
		memcpy(br->manual_clone_mac, macaddr, 6);
		memset(br->auto_clone_mac, 0x0, ETH_ALEN);
		return 0;
	}
	...
	case 147:
	{
		...
		if(br->mat_mode == 1)		memcpy(macaddr, br->auto_clone_mac, 6);
		else if(br->mat_mode == 2)	memcpy(macaddr, br->manual_clone_mac, 6);
		else						memcpy(macaddr, br->dev->dev_addr, 6);
		if (copy_to_user((void __user *)args[1], macaddr, 6));
			return -EFAULT;
		return 0;
	}
