...
static void ipmr_expire_process(unsigned long dummy)
{
	...
	while ((c=*cp) != NULL) {
		...
		*cp = c->next;
		...
		mcast.s_addr = c->mfc_mcastgrp;
		printk(KERN_INFO "After calling ipmr_destroy_unres(), mfc_mcastgrp: %u.%u.%u.%u \n",NIPQUAD(mcast.s_addr) );
		#endif
	}
...
static struct mfc_cache *ipmr_cache_alloc_unres(void)
{
	...
	c->mfc_un.unres.expires = jiffies + ( (1*HZ)/100 );
	return c;
}
