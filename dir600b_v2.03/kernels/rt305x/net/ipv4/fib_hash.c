 ...
 *
 * Version:	$Id: fib_hash.c,v 1.1.1.1 2007-05-25 06:49:59 bruce Exp $
 *
...
static inline void fn_free_alias(struct fib_alias *fa)
{
		...
		if (hw_tcpip && hw_tcpip->del_hw_fib_entry)
				hw_tcpip->del_hw_fib_entry(fi);
#endif
...
#ifdef CONFIG_HARDWARE_TURBO
		if (hw_tcpip&&hw_tcpip->add_hw_fib_entry)
		{
			struct rtmsg *r=0;
			struct kern_rta *rta=0;
			hw_tcpip->add_hw_fib_entry(tb,r,rta,fi);
		}
#endif
