...
extern unsigned int ip_ct_all_protocol_cleanup;
#endif
...
static ctl_table ip_ct_sysctl_table[] = {
...
#if     defined(CONFIG_IP_NF_CONNTRACK_CLEAN)
	{
		.ctl_name	= NET_IPV4_NF_CONNTRACK_TCP_IPS_QOS_HI_LEN,
		.procname	= "ip_conntrack_tcp_ips_qos_hi_len",
		.data		= &ips_qos_hi_len,
		.maxlen		= sizeof(int),
		.mode		= 0444,
		.proc_handler	= &proc_dointvec,
	...
	{
		.ctl_name	= NET_IPV4_NF_CONNTRACK_TCP_IPS_QOS_LEN,
		.procname	= "ip_conntrack_tcp_ips_qos_len",
		.data		= &ips_qos_len,
		.maxlen		= sizeof(int),
		.mode		= 0444,
		.proc_handler	= &proc_dointvec,
	...
	{
		.ctl_name	= NET_IPV4_NF_CONNTRACK_ALL_PROTOCOL_CLEANUP,
		.procname	= "ip_conntrack_all_protocol_cleanup",
		.data		= &ip_ct_all_protocol_cleanup,
		.maxlen		= sizeof(unsigned int),
		.mode		= 0644,
		.proc_handler	= &proc_dointvec_jiffies,
	},
...
void need_conntrack(void)
{
   ...
   EXPORT_SYMBOL(ip_conntrack_max);
   EXPORT_SYMBOL(ip_conntrack_checking);
#endif

#if defined (CONFIG_IP_NF_QOS) || defined (CONFIG_IP_NF_TARGET_STUN)
extern atomic_t 	ip_conntrack_amount_atomic;
EXPORT_SYMBOL(ip_conntrack_amount_atomic);
#endif
...
EXPORT_SYMBOL(ip_ct_tcp_timeout_established);
EXPORT_SYMBOL(ip_ct_udp_timeout_stream);
