			       ...
			       struct ip_conntrack_expect *exp)
{
	...
	/* We MUST call ip_conntrack_h245_expect() before ip_nat_redirect(),
	 * because ip_nat_redirect() will call ip_nat_setup_info() which will call
	 * sw_add_NAT_entry() if fastnat module is enable. In sw_add_NAT_entry() function,
	 * If conntrack->helper is NULL, this packet will be added to fastnat table, and 
	 * follow packets will be forwarded by fastnat module and not forward to correct 
	 * dest IP.
	 *           - kwest,2009/5/6
	 */
	//ip_conntrack_h245_expect(new, exp);
			       ...
			       struct ip_conntrack_expect *exp)
{
	...
	ip_nat_redirect(new, exp);
	/* We MUST call ip_conntrack_q931_expect() before ip_nat_redirect(),
	 * because ip_nat_redirect() will call ip_nat_setup_info() which will call
	 * sw_add_NAT_entry() if fastnat module is enable. In sw_add_NAT_entry() function,
	 * If conntrack->helper is NULL, this packet will be added to fastnat table, and 
	 * follow packets will be forwarded by fastnat module and not forward to correct 
