...
#if defined(CONFIG_IP_NF_QOS)
unsigned int ip_ct_tcp_timeout_established __read_mostly =   TCP_TIMEOUT_ESTABLISH_NORMAL SECS;
#else
...
#ifdef CONFIG_HARDWARE_TURBO
	if (hw_tcpip && hw_tcpip->refresh_tcp_connection)
		hw_tcpip->refresh_tcp_connection(th, conntrack);
#endif
...
#ifdef CONFIG_HARDWARE_TURBO
		if (!hw_tcpip && !tcp_in_window(&conntrack->proto.tcp, dir, index,
#else
