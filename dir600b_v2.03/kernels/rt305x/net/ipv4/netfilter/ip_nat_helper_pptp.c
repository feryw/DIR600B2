			      ...
			      struct ip_conntrack_expect *exp)
{
	...
	if (exp->dir == IP_CT_DIR_REPLY) {
		/*
		* To fix PPTP/Russia PPTP can't reconnect to D-Link DFL series firewall issue.
		* 
		* When our pptp first connect to DFL firewall, DFL firewall always use 0 as call id, so
		* netfilter will use 0~1 as tuple range_size which means it only support one tuple.
		* and when reconnect to fireall, the previous gre conntrack has not been clear 
		* (BUG? 2.6 kernel not clear it but 2.4 kernel will). so netfilter will get the duplicate tuple which 
		* has been used by previous connection, then this connection can't get NAT mapping which result to 
		* reconnect to firewall failed.
		* 
		*	- Kwest 2009/02/25
		*
		* Restore below line to orignal, because privious modification would cause to WinXP PPTP client can't 
		* connect to DFL-1600/DFL-200/DFL-900 PPTP server.
		*
		*	- Kwest 2009/06/18
		*/
		range.flags |= IP_NAT_RANGE_PROTO_SPECIFIED;
