...
#include <linux/netfilter_ipv4/ip_conntrack_core.h>
   ...
   extern int  *stun_list_account;
	...
	EXPORT_SYMBOL(ips_qos_len);
	EXPORT_SYMBOL(ips_qos_hi_len);
#endif
	...
	case IP_CT_NEW:
		...
		if (!ip_nat_initialized(ct, maniptype)) {
			...
			if (unlikely(is_confirmed(ct)))
               ...
               if(stun_helper&&stun_helper->stun_manip_packet
                   /*&&((*stun_list_account)>0)*/) {
                   
                   /* Checking if STUN-bit be set */
                   if(test_bit(IPS_STUN_BIT, &ct->status)) {
                       unsigned int    manip;
                       manip=stun_helper->stun_manip_packet(ct, hooknum, pskb);
                       switch(manip) {
                       case NF_DROP:
                           printk("STUN incoming packet drop\n");
                           return  manip;
                       case NF_ACCEPT:
                           printk("STUN incoming packet accept\n");
/*
 * If a packet send from router to LAN, the prio should be HIGH, but below lines change the prio to MIDDLE,
 * that is not correct. I comment these lines to fix this bug.
 * So if a incoming packet send from WAN to LAN (not LAN->WAN reply packet), comment below lines will not modify these packet prio
 * to MIDDLE, it just keep its HIGH prio. In other words, Virtual Server and DMZ have HIGH prio.
 *				-kwest 
 */
#if		defined(CONFIG_IP_NF_QOS)
								{
                       ...
                       default:
                           /* Not possible */
                           printk(KERN_WARNING "STUN incoming packets error!\n");
                           return  NF_DROP;
                       }
                   }
               }
#endif //CONFIG_IP_NF_TARGET_STUN  //alan
