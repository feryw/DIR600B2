...
unsigned int ip_pers_sport = 0;
unsigned int ip_pers_enable = 0;

EXPORT_SYMBOL(ip_pers_enable);
EXPORT_SYMBOL(ip_pers_sport);
#endif
			       ...
			       u32 *seq)
{
	...
	exp->dir = !dir;
#ifdef CONFIG_IP_NF_PERS
	if (ip_pers_enable==1 && ip_pers_sport > 0) {
		if (type == IP_CT_FTP_PORT || type == IP_CT_FTP_EPRT) {
			port = ip_pers_sport;
		}
	}
#endif
