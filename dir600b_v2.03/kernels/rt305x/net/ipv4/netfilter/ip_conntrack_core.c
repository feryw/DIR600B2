...
#ifdef CONFIG_IP_NF_TARGET_STUN
   #include <linux/netfilter_ipv4/ipt_STUN.h>
   extern int  *stun_list_account;
#endif
#if defined (CONFIG_IP_NF_QOS) || defined (CONFIG_IP_NF_TARGET_STUN)
	atomic_t	ip_conntrack_amount_atomic=ATOMIC_INIT(0);
#endif
...
#if		defined(CONFIG_IP_NF_QOS) || defined(CONFIG_IP_NF_CONNTRACK_CLEAN)
...
clean_from_lists(struct ip_conntrack *ct)
{
	DEBUGP("clean_from_lists(%p)\n", ct);
		...
		if(hw_tcpip && hw_tcpip->del_hw_nat_entry) {
				...
				DEBUGP("Clean_from_lists: do hardware cleanup\n");
				if((ret = hw_tcpip->del_hw_nat_entry(ct)) == -EPERM) {
						DEBUGP("ip_nat_cleanup_conntrack(%ld): retrun(%d)\n", ct->timeout.expires, ret);
						return;
				}
		}
#endif
...
destroy_conntrack(struct nf_conntrack *nfct)
{
...
#ifdef CONFIG_IP_NF_TARGET_STUN
       {
           if(stun_helper&&stun_helper->stun_destory&&((*stun_list_account)>0)) {
               uint16_t    protonum=ct->tuplehash[IP_CT_DIR_REPLY].tuple.dst.protonum;
               
               if(protonum==IPPROTO_UDP||protonum==IPPROTO_TCP) {
                   SESSION_FLOW    src, dst, mvp, map;
                   uint16_t        *src_p, *dst_p, *mvp_p, *map_p;

                   src.ip=ct->tuplehash[0].tuple.src.ip;
                   dst.ip=ct->tuplehash[0].tuple.dst.ip;
                   mvp.ip=ct->tuplehash[1].tuple.dst.ip;
                   map.ip=ct->tuplehash[1].tuple.src.ip;
                   src_p=(uint16_t *)&ct->tuplehash[0].tuple.src.u.all;
                   dst_p=(uint16_t *)&ct->tuplehash[0].tuple.dst.u.all;
                   mvp_p=(uint16_t *)&ct->tuplehash[1].tuple.dst.u.all;
                   map_p=(uint16_t *)&ct->tuplehash[1].tuple.src.u.all;
                   /* __LITTLE_ENDIAN && __BIG_ENDIAN */
                   src.port=ntohs(*(src_p+0));
                   dst.port=ntohs(*(dst_p+0));
                   mvp.port=ntohs(*(mvp_p+0));
                   map.port=ntohs(*(map_p+0));
                   stun_helper->stun_destory(&src, &mvp, &dst, protonum);
                   stun_helper->stun_destory(&map, &dst, &src, protonum);
               }
           }
       }
#endif //CONFIG_IP_NF_TARGET_STUN
	...
	if (!is_confirmed(ct)) {
		...
		int		prio=0;
		int		keeptimeout=-1;
		keeptimeout=ips_qos_get_keeptimeout(ct);
		prio=ips_qos_get_priority(ct);
		if(keeptimeout==-1 || prio==0)
			IPS_QOS_ERROR;
		ips_qos_alter_len(prio, keeptimeout, IPS_QOS_DEL);
		...
		u32to8(&sip, &psip_8);
		u32to8(&dip, &pdip_8);
		printk("destroy_conntrack: (-pr=%d, kp=%d)%d.%d.%d.%d -> %d.%d.%d.%d [0x%X]\n"
			, prio, keeptimeout
...
#ifdef    CONFIG_IP_NF_TARGET_STUN
/* Return 1 if the ip_conntrack object is still exist
 * Otherwise, return 0
 */
short  ip_conntrack_checking(SESSION_FLOW ori_src, 
                           SESSION_FLOW ori_map, 
                           SESSION_FLOW ori_dst, 
                           uint16_t ori_protonum)
{
   int     i=0, retval=0;
   for(; i<ip_conntrack_htable_size; i++) {
       if(list_empty(&ip_conntrack_hash[i]))
           ...
           struct ip_conntrack_tuple_hash  *pos=NULL;
           list_for_each_entry(pos, &ip_conntrack_hash[i], list) {
               uint16_t        protonum=pos->tuple.dst.protonum;
               SESSION_FLOW    src, map, dst;
               
               if(protonum!=ori_protonum)
                   continue;
               src.ip=pos->tuple.src.ip;
               dst.ip=pos->tuple.dst.ip;
               
               /* __LITTLE_ENDIAN && __BIG_ENDIAN */
               if(protonum==IPPROTO_TCP) {
                   src.port=ntohs(pos->tuple.src.u.tcp.port);
                   dst.port=ntohs(pos->tuple.dst.u.tcp.port);
               } else if(protonum==IPPROTO_UDP) {
                   src.port=ntohs(pos->tuple.src.u.udp.port);
                   dst.port=ntohs(pos->tuple.dst.u.udp.port);
               } else {
                   /* Don't support */
                   STUN_ERROR;
               }
               if(src.ip==ori_src.ip&&src.port==ori_src.port
                   &&dst.ip==ori_dst.ip&&dst.port==ori_dst.port) {
                   struct ip_conntrack         *ct;
                   struct ip_conntrack_tuple   *new;
                   
                   ct=tuplehash_to_ctrack(pos);    /* Differ from 2.4 */
                   new=&ct->tuplehash[1].tuple;
                   map.ip=new->dst.ip;

                   /* __LITTLE_ENDIAN && __BIG_ENDIAN */
                   if(protonum==IPPROTO_TCP) {
                       map.port=ntohs(new->dst.u.tcp.port);
                   } else if(protonum==IPPROTO_UDP) {
                       map.port=ntohs(new->dst.u.udp.port);
                   } else {
                       /* Dont support */
                       STUN_ERROR;
                       continue;
                   }
                   if(map.ip==ori_map.ip&&map.port==ori_map.port) {
                       retval=1;/*Exist*/
                       goto    RET;
                   }
               }
...
static int early_drop(struct list_head *chain)
{
	...
	/* SHOULD add some lock to avoid completing, modified by Enos */
	write_lock_bh(&ip_conntrack_lock);
...
#ifdef    CONFIG_IP_NF_TARGET_STUN
   atomic_inc(&ip_conntrack_amount_atomic);
#endif
#if		defined(CONFIG_IP_NF_QOS)
	/* Check out traffic type */
	int		keeptimeout=0;
	int		prio=0;
	prio=ips_qos_check_entry(orig, &keeptimeout);
	//keeptimeout=ips_qos_get_keeptimeout(conntrack);
	if(keeptimeout && atomic_read(&ips_qos_hi_len)<IPS_QOS_HI_MAX_LEN) {
		goto	ALLOC_CONN;
	}
	if(ips_qos_max_len<=atomic_read(&ips_qos_len)) {
		goto	EARLY_DROP;
	}
#endif
...
#if		defined(CONFIG_IP_NF_QOS) /* kwest: move label to here to prevent early_drop() kernel panic */
EARLY_DROP:
#endif
...
#ifdef    CONFIG_IP_NF_TARGET_STUN
			atomic_dec(&ip_conntrack_amount_atomic);
#endif
			if (net_ratelimit())
				printk(KERN_WARNING
...
ip_conntrack_free(struct ip_conntrack *conntrack)
{
   ...
   atomic_dec(&ip_conntrack_amount_atomic);
#endif
#if	0 // defined(CONFIG_IP_NF_QOS) /* kwest: It should not be called here, I move it to destroy_conntrack(). */
	{
		int		prio=0;
		int		keeptimeout=-1;
		keeptimeout=ips_qos_get_keeptimeout(conntrack);
		prio=ips_qos_get_priority(conntrack);
		if(keeptimeout==-1 || prio==0)
			IPS_QOS_ERROR;
		ips_qos_alter_len(prio, keeptimeout, IPS_QOS_DEL);
	}
...
#if		defined(CONFIG_IP_NF_CONNTRACK_CLEAN)
static int			flag_all_timeout_all_state = 0;
unsigned int	ip_ct_all_protocol_cleanup = 0;
/* tune_all_timeout_all_state(): 
 * explore all ct and modify the timeout of ct with all state to the input argument 
 *
 * Input: timeout secs 
 * Output: Null
 */
static void	tune_all_timeout_all_state(int timeout, 
						struct sk_buff *skb, 
						struct ip_conntrack_tuple *tuple)
{
	int			index=0;
	...
	if(flag_all_timeout_all_state)
		return;
	
	read_lock_bh(&ip_conntrack_lock); 
	for(; index<ip_conntrack_htable_size; index++) {
		if(list_empty(&ip_conntrack_hash[index]))
			continue;
		else {
			struct ip_conntrack_tuple_hash	*pos=NULL;
			list_for_each_entry(pos, &ip_conntrack_hash[index], list) {
				ct = tuplehash_to_ctrack(pos);
				//FIX ME: skb is not useful in ip_ct_refresh() function, for event cache is not enabled.
				ip_ct_refresh(ct, skb, 0);
			}
		}
	}
	read_unlock_bh(&ip_conntrack_lock); 
										...
										struct ip_conntrack_tuple *tuple)
{
	...
	int			index=0;
	struct ip_conntrack *ct;
	ip_ct_tcp_timeout_established=timeout*HZ;
	...
	read_lock_bh(&ip_conntrack_lock); //kwest++
	for(; index<ip_conntrack_htable_size; index++) {
		if(list_empty(&ip_conntrack_hash[index]))
			continue;
		else {
			...
			list_for_each_entry(pos, &ip_conntrack_hash[index], list) {
				...
				case	IPPROTO_TCP:
					...
					if(ct->proto.tcp.state==TCP_CONNTRACK_ESTABLISHED) {
						if(IPS_QOS_TEST_BIT(IPS_QOS_KEEP_BIT, &pos->tuple.ips_qos_type)
							|| IPS_QOS_TEST_BIT(IPS_QOS_HI_BIT, &pos->tuple.ips_qos_type)) {
#if		IPS_QOS_DEBUGMOD
							++hi;
#endif
						} else {
							//FIX ME: skb is not useful in ip_ct_refresh() function, for event cache is not enabled.
							ip_ct_refresh(ct, skb, ip_ct_tcp_timeout_established);
#if		IPS_QOS_DEBUGMOD
							++count_5;
#endif
						}
	...
	read_unlock_bh(&ip_conntrack_lock); //kwest++
}
#endif
...
#if		defined(CONFIG_IP_NF_QOS)
	/* Change ip ct tcp establish timeout to 5 if ct > 1024
	 * Change ip ct tcp establish timeout to 7200 if ct < 768
 	 */
#if	defined(CONFIG_IP_NF_CONNTRACK_CLEAN)
	/* Add some check to support failover, by Enos+++ */
	if(ip_ct_all_protocol_cleanup == 0) {
#endif
		if(atomic_read(&ip_conntrack_count)>TCP_TRAFFIC_THROTTLE 
			&& !flag_tcp_timeout_establish) {
			tune_tcp_timeout_establish(TCP_TIMEOUT_ESTABLISH_BT, skb, tuple);
			flag_tcp_timeout_establish=1;
		} else if(atomic_read(&ip_conntrack_count)<TCP_TRAFFIC_LOW 
			&& flag_tcp_timeout_establish) {
			tune_tcp_timeout_establish(TCP_TIMEOUT_ESTABLISH_NORMAL, skb, tuple);
...
#ifdef CONFIG_IP_NF_TARGET_STUN
   if(stun_helper&&stun_helper->stun_check_packet/*&&((*stun_list_account)>0)*/) {
       
       /* Pick out STUN packets from input traffic of WAN */
       if(!exp/*&&(!conntrack->helper)*/) {
           uint16_t    protonum=tuple->dst.protonum;
           
           if(protonum==IPPROTO_UDP||protonum==IPPROTO_TCP) {
               SESSION_FLOW    map;
               SESSION_FLOW    dst;
               uint16_t    *x=(uint16_t *)&tuple->dst.u.all;
               uint16_t    *y=(uint16_t *)&tuple->src.u.all;
               
               dst.ip=tuple->src.ip;
               map.ip=tuple->dst.ip;
/*#if        LINUX_VERSION_CODE<KERNEL_VERSION(2, 6, 0)
               #error  Version of kernel is ERROR
#endif*/
               /* __LITTLE_ENDIAN && __BIG_ENDIAN */
               map.port=ntohs(*(x+0));
               dst.port=ntohs(*(y+0));

               if(!stun_helper->stun_check_packet(&map, &dst, protonum)) {
                   /* atomic operation */
                   set_bit(IPS_STUN_BIT, &conntrack->status);
               }
           }
       }
   }
	...
	int		keeptimeout=0;
	int		prio=0;
	//prio=ips_qos_check_entry(tuple, &keeptimeout); /* kwest: this line will cause negative counter value */
	prio=ips_qos_check_entry(&conntrack->tuplehash[IP_CT_DIR_ORIGINAL].tuple, &keeptimeout);
	ips_qos_modify_qostype_2_ct(prio, keeptimeout, conntrack);
	ips_qos_alter_len(prio, keeptimeout, IPS_QOS_ADD);

#if     IPS_QOS_DEBUGMOD
		uint32_t	sip, dip;
		uint8_t	*psip_8, *pdip_8;
		
		sip=conntrack->tuplehash[IP_CT_DIR_ORIGINAL].tuple.src.ip;
		dip=conntrack->tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.ip;
		
		u32to8(&sip, &psip_8);
		u32to8(&dip, &pdip_8);
		printk("init_conntrack:    (+pr=%d, kp=%d)%d.%d.%d.%d -> %d.%d.%d.%d [0x%X]\n"
			, prio, keeptimeout
			, *(psip_8+0), *(psip_8+1), *(psip_8+2), *(psip_8+3)
			, *(pdip_8+0), *(pdip_8+1), *(pdip_8+2), *(pdip_8+3)
			, conntrack->tuplehash[IP_CT_DIR_ORIGINAL].tuple.ips_qos_type);
#endif
#endif
...
#if	defined(CONFIG_IP_NF_CONNTRACK_CLEAN)
	/* Failover occured, we should clean all conntracks. by Enos+++ */
	if(ip_ct_all_protocol_cleanup != 0){ /* MUST NOT BE 0, but SHOULD ALSO BE 1. */
		/* We check flag_all_timeout_all_state flag to see if conntracks cleanning was done or not. */
		if(flag_all_timeout_all_state == 0) {
			tune_all_timeout_all_state(0, skb, &tuple);
			flag_all_timeout_all_state = 1;
#if		defined(CONFIG_IP_NF_QOS)
			flag_tcp_timeout_establish = 1;
#endif
		}
	} else {
		flag_all_timeout_all_state = 0;
	}
#endif
			...
			int do_acct)
{
...
#if		defined(CONFIG_IP_NF_QOS)
	/*if(	IPS_QOS_TEST_BIT(IPS_QOS_KEEP_BIT
		, (void *)&ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.ips_qos_type) 
		|| IPS_QOS_TEST_BIT(IPS_QOS_HI_BIT
		, (void *)&ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.ips_qos_type)) {
		//extra_jiffies=TCP_TIMEOUT_ESTABLISH_NORMAL*HZ;
		if(extra_jiffies==TCP_TIMEOUT_ESTABLISH_BT) {
			extra_jiffies=TCP_TIMEOUT_ESTABLISH_NORMAL*HZ;
		}
	}*/
	...
	case	IPPROTO_TCP:
		if(IPS_QOS_TEST_BIT(IPS_QOS_KEEP_BIT
			...
			if(extra_jiffies==TCP_TIMEOUT_ESTABLISH_BT*HZ) {
				extra_jiffies=TCP_TIMEOUT_ESTABLISH_NORMAL*HZ;
				IPS_QOSDBG("Tcp establish- %d\n", extra_jiffies);
			}
		}
		break;
	default:
