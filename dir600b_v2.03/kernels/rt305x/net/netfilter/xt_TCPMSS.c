...
tcpmss_mangle_packet(struct sk_buff **pskb,
		     ...
		     const struct net_device *in, /*kwest++*/
		     ...
		     unsigned int minlen)
{
	...
	if (info->mss == XT_TCPMSS_CLAMP_PMTU) {
		...
		/* Orginal implementation just clamp PMTU accroding to dst MTU,
		 * which don't consider incoming interface. So, I clamp PMTU between 
		 * incoming interface and outgoing interface, it is better for our NAT router.
		 *  --kwest
		 */
		if (in) {
			if (in->mtu <= minlen) {
		  ...
		  const void *targinfo)
{
	...
	ret = tcpmss_mangle_packet(pskb, targinfo, in, iph->ihl * 4,
		  ...
		  const void *targinfo)
{
	...
	ret = tcpmss_mangle_packet(pskb, targinfo, in, tcphoff,
