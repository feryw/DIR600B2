...
#ifdef CONFIG_HARDWARE_TURBO 
		if (hw_tcpip && hw_tcpip->check_hw_reference) {
				if(!hw_tcpip->check_hw_reference(neigh)) {
						return;
				}
		}
#endif
...
#ifdef CONFIG_HARDWARE_TURBO 
		NEIGH_PRINTK2("___NEIGH DEBUG %p hh %p is free.\n", neigh, hh);
		if (hw_tcpip && hw_tcpip->del_hw_arp_entry) {
				if(hw_tcpip->del_hw_arp_entry(neigh)) {
						//printk("Neigh State not match\n");
				}
				//printk("D-neigh->nud_state(0x%x) tbl->id = 0x%u\n", neigh->nud_state, neigh->tbl->id);
		}
#endif
		 ...
		 u32 flags)
{
	...
	if (lladdr != neigh->ha) {
				...
				if (hw_tcpip && hw_tcpip->del_hw_arp_entry)
						hw_tcpip->del_hw_arp_entry(neigh);
#endif
		memcpy(&neigh->ha, lladdr, dev->addr_len);

#ifdef CONFIG_HARDWARE_TURBO 
				NEIGH_PRINTK2("___neigh %p hardware update.\n", neigh);
				if(hw_tcpip && hw_tcpip->add_hw_arp_entry && (flags & NEIGH_UPDATE_F_ADMIN)) {
						//printk("-U-neigh->nud_state(0x%x) tbl->id = 0x%x\n", neigh->nud_state, neigh->tbl->id
						hw_tcpip->add_hw_arp_entry(neigh, lladdr);
				}
#endif
