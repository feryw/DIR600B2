...
"Overview\n"
"--------\n"
"Some features may be built directly into ELBOX.  Some features\n"
...
load_config_help[] =
	"\n"
	"For various reasons, one may wish to keep several different ELBOX\n"
	"configurations available on a single machine.\n"
	"\n"
	"If you have saved a previous configuration in a file other than the\n"
	"ELBOX's default, entering the name of the file here will allow you\n"
...
save_config_help[] =
	"\n"
	"For various reasons, one may wish to keep different ELBOX\n"
	"configurations available on a single machine.\n"
	"\n"
...
static void show_textbox(const char *title, const char *text, int r, int c)
{
	...
	dummy = write(fd, text, strlen(text));
	close(fd);
...
int main(int ac, char **av)
{
	...
	snprintf(menu_backtitle, 128, "ELBOX Configuration");
	...
	do {
		stat = dialog_yesno(NULL,
				    "Do you wish to save your new ELBOX configuration?", 5, 60);
	} while (stat < 0);
	end_dialog();
	...
	if (stat == 0) {
		conf_write(NULL);
		printf("\n\n"
			"*** End of ELBOX configuration.\n"
			"*** Check the top-level Makefile for additional configuration options.\n\n");
	} else
		printf("\n\nYour ELBOX configuration changes were NOT saved.\n\n");
