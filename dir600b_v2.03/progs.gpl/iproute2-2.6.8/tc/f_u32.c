...
#if ELBOX_PROGS_GPL_TC_U32_IP_PORT_RANGE /*Erick*/
        if (off<12||off>20)
#endif 
	key &= mask;
...
static __u16 u16_cpu_to_be(__u16 value)
{
	static int swap = -1;
	static __u16 patt = 0x0102;
	static __u8 * p = (__u8 *)&patt;
...
static int pack_key16(struct tc_u32_sel *sel, __u32 key, __u32 mask, int off, int offmask)
{
	if (key > 0xFFFF || mask > 0xFFFF)
		return -1;
#if ELBOX_PROGS_GPL_TC_U32_IP_PORT_RANGE /*Erick*/
    if ((off==20||off==22)&&mask!=0)
    {
        mask += key;
        if (mask>0xFFFF)
            mask=0xFFFF; 
        
        /*To solve Endian problem, Builder, 20080508*/
        /*+++*/
        key = u16_cpu_to_be(key);
        mask = u16_cpu_to_be(mask);
        /*+++*/
...
static __u32 u32_cpu_to_be(__u32 value)
{
	static int swap = -1;
	static __u32 patt = 0x01020304;
	static __u8 * p = (__u8 *)&patt;
	...
	if (addr.bitlen)
        {
            ...
            /*To solve Endian problem, Builder, 20080508*/
            /*+++*/    
            mask = u32_cpu_to_be(addr.bitlen);
            /*+++*/
#else
