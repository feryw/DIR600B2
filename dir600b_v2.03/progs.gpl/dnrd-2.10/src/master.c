...
#define	MASTER_CONFIG		"/var/dnrd/master"
...
static int _master_init(void)
{
		...
		(read_configuration(config) != 0))
	{
		...
		*domain = 0;
		if ((fp = fopen("/etc/resolv.conf", "r")) != NULL)
		{
			...
			log_debug("initialising from /etc/hosts, domain= %s", *domain == 0? "<none>": domain);
	...
	if (auto_authority != 0)
	{
		...
		for (i = 0; i < dbc; i++)
		{
			...
			{
				snprintf (arpaname, sizeof(arpaname) - 2, "%s%s", dbv[i]->u.nameip.arpa.string, ARPADOMAIN);
...
int master_lookup(unsigned char *msg, int len)
{
	...
	log_debug("master_lookup!!\n");
	
	if (master_onoff == 0)
	{
		log_debug("master off!\n");
		return (0);
	}
	...
	else if (query.type == DNS_TYPE_A || query.type == DNS_TYPE_AAAA)
	{
		dnsheader_t *x=NULL;
		log_debug("type = DNS_TYPE_%s [%s]\n", query.type == DNS_TYPE_A?"A":"AAAA", query.name);
		...
		else if(serv_cnt==0 && serv2_cnt==0 && recv_addr.sin_addr.s_addr)	//-----no server was given, response device's ip to client
		{
			x = begin_assembly(&query);
			compile_objectname(x);
			compile_int(x, DNS_TYPE_A);
			compile_int(x, DNS_CLASS_INET);
			compile_long(x, 0);	//-----defined TTL to 0 to avoid cache.
			start_rdata(x);
			compile_long(x, ntohl(recv_addr.sin_addr.s_addr));
			end_rdata(x);
		}
	...
	else
	{
		 ...
		 * by the master.
		 */
		log_debug("master: query type(%X) is not handled by the master\n", query.type);
		return (0);
	}
...
int master_init(void)
{
	...
	/*
	 * Since our root dir will have changed to /var/dnrd, we need to change
