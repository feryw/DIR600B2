...
#define def_Without_fork

#define BINDING_RETRY	3
/*
...
int main(int argc, char *argv[])
{
	...
	for (sl = keylist; sl; sl = sl->next)
	{
		sl->cname = convert_name_string(sl->string);
	}
...
#if 0	/* for debug */
	{
		...
		char * sample1 = "www.flets";
		char * sample2 = "www.flets.com";
		char * sample3 = "www.corega.flets.com";
		char * test = NULL;

		printf("DNS servers (%d):\n", serv_cnt);
		for (i=0; i<serv_cnt; i++)
		{
			printf("%08x\n", dns_srv[i].addr.sin_addr);
		}
		printf("DNS session2:\n");
		for (i=0; i<serv2_cnt; i++)
		{
			printf("%08x\n", dns_srv2[i].addr.sin_addr);
		}

		printf("KEYs for session2:\n");
		while (sl)
		{
			printf("key: [%s]\n", sl->string);
			if (sl->cname)
			{
				for (i=0; sl->cname[i]; i++)
				{ printf("%02x ", (unsigned char)sl->cname[i]); }
				printf("\n");
			}
			else
			{
		...
		test = convert_name_string(sample1);
		i = match_keylist(test);
		printf("%s is %s\n", sample1, i ? "matched" : "not matched");
		test = convert_name_string(sample2);
		i = match_keylist(test);
		printf("%s is %s\n", sample2, i ? "matched" : "not matched");
		test = convert_name_string(sample3);
		i = match_keylist(test);
		printf("%s is %s\n", sample3, i ? "matched" : "not matched");
	...
	if (!filep)
	{
		log_msg(LOG_ERR, "can't write to %s. Check that dnrd was started by root.", pid_file);
		cleanexit(-1);
	}
	...
	{
		log_msg(LOG_ERR, "domain ports for udp & tcp differ. Check /etc/services");
		cleanexit(-1);
	}
	...
	{
	/*
	 * sometimes dnrd will fail to bind local address, we retry...
	 * Leon.
	 */
	int j = BINDING_RETRY;
	...
	do {
		if (bind(tcpsock, (struct sockaddr *)&recv_addr, sizeof(recv_addr)) < 0)
		{
			...
			log_msg(LOG_ERR, "tcpsock: sleep 1 sec... try again...");
			sleep(1);
			j--;
			continue;
		}	
	...
	if (j <=0)
	{
		log_msg(LOG_ERR, "tcpsock: Dnrd still fail,give up");
		cleanexit(-1);
	}	
	}
	...
	if (!dirp)
	{
		log_msg(LOG_ERR, "The directory /var/dnrd must be created before dnrd will run");
	}
	...
	{
		log_msg(LOG_ERR, "The /var/dnrd directory should only be user writable");
		cleanexit(-1);
	}
...
*/
	...
	/*
	 * Setup DNS for session2
	 */
	for (i = 0; i < serv2_cnt; i++)
	{
		if ((dns_srv2[i].sock = socket(AF_INET, SOCK_DGRAM, 0)) < 0)
		{
			log_msg(LOG_ERR, "osock: Couldn't open socket");
			cleanexit(-1);
		}
		dns_srv2[i].addr.sin_family = AF_INET;
		dns_srv2[i].addr.sin_port	= htons(53);
	}
	...
	/* Run forever. */
	run();
	cleanexit(0); /* to make compiler happy */
	return 0;
}
