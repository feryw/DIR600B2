...
parse_to(char *arg, int portok, struct ipt_natinfo *info, int type)
{
	...
	if (colon) {
		...
		if (error)
			exit_error(PARAMETER_PROBLEM, "Invalid port:port syntax - use dash\n");
		...
		if (!dash) {
			if(type==IPT_DNAT_TO_DEST) {
				...
				if(shift<=0 || shift>65535)
					exit_error(PARAMETER_PROBLEM, "Shift `%s' not valid\n", dash+1);
...
#else	
		/* For now, no ip is unvalid */
		if(colon==arg && type==IPT_DNAT_TO_DEST)
			return &(append_range(info, &range)->t);
		else if(colon==arg && type==IPT_DNAT_TO_SHIFT)
			goto	SHIFT_ERR;
		else
			;;
#endif
	...
	return &(append_range(info, &range)->t);
SHIFT_ERR:
	exit_error(PARAMETER_PROBLEM, "Must specify '--to-shift ip:offset'\n");
}
      ...
      struct ipt_entry_target **target)
{
	...
	case '1':
		if (check_inverse(optarg, &invert, NULL, 0))
			exit_error(PARAMETER_PROBLEM, "Unexpected `!' after --to-destination");

		if (*flags&IPT_DNAT_TO_DEST) {
			if (!kernel_version)
				get_kernel_version();
			if (kernel_version > LINUX_VERSION(2, 6, 10))
				exit_error(PARAMETER_PROBLEM, "Multiple --to-destination not supported");
		}
		dnat_type=IPT_DNAT_TO_DEST;
		break;
	case '2':
		if(check_inverse(optarg, &invert, NULL, 0))
			exit_error(PARAMETER_PROBLEM, "Unexpected `!' after --to-shift");
		if(*flags&IPT_DNAT_TO_SHIFT) {
			exit_error(PARAMETER_PROBLEM, "--to-shift only once!");
		}
		dnat_type=IPT_DNAT_TO_SHIFT;
		break;
...
OK:
	*flags|=dnat_type;
	*target=parse_to(optarg, portok, info, dnat_type);
	dnatok=1;
ERROR:
	return	dnatok;
}
...
{
	if (!flags)
		exit_error(PARAMETER_PROBLEM, "You must specify --to-destination/--to-shift");
}
