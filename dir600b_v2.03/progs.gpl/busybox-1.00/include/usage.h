...
#define ftpget_trivial_usage \
	...
	"\t [options] remote-host remote-file (for F/W and configuration)\n"
#else /*ftp_tftp_FW_CG_20070125 Erick*/
#define ftpget_trivial_usage \
	"[options] remote-host local-file remote-file"
#endif
#if ELBOX_PROGS_GPL_TFTP_FTP_GET_PUT_FW_CONFIG /*ftp_tftp_FW_CG_20070125 Erick*/
#define ftpget_full_usage \
	...
	"\t-f, --firmware         Get F/W and apply it to flash\n" \ 
	"\t-d, --get CFG          get configuration file and apply it\n" \
	"\t-v, --verbose          Verbose\n" \
	"\t-u, --username         Username to be used\n" \
	"\t-p, --password         Password to be used\n" \
	"\t-P, --port             Port number to be used\n" \
	"\t-m, --get ACL          Get acl file and apply it to xmldb"
#else /*ftp_tftp_FW_CG_20070125 Erick*/
#define ftpget_full_usage \
	"Retrieve a remote file via FTP.\n\n" \
	"Options:\n" \
...
#define ftpput_trivial_usage \
	...
	"\t [options] remote-host remote-file (for configuration)"
#else /*ftp_tftp_FW_CG_20070125 Erick*/
#define ftpput_trivial_usage \
	"[options] remote-host remote-file local-file"
#endif
...
#define ftpput_full_usage \
	...
	"Options:\n" \
	"\t-t, --put CFG          copy current configuration file to server\n" \
	"\t-v, --verbose          Verbose\n" \
	"\t-u, --username         Username to be used\n" \
	"\t-p, --password         Password to be used\n" \
	"\t-P, --port             Port number to be used\n"  \
	"\t-n, --put ACL          copy current acl file to server"
#else
#define ftpput_full_usage \
	"Store a local file on a remote machine via FTP.\n\n" \
	"Options:\n" \
...
#define tftp_full_usage \
	...
	"\t-r FILE\tRemote FILE." \
        ...
        "\n\t-g\tGet file." \
        "\n\t-f\tGet F/W." \
        "\n\t-d\tGet configuration file." \
        "\n\t-m\tGet ACL file." \
        ) \
        USAGE_TFTP_PUT(	\
	"\n\t-p\tPut file." \
	"\n\t-t\tPut configuration file." \
	"\n\t-n\tPut ACL file."\
	) \
	USAGE_TFTP_BS( \
