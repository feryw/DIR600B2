...
function wmm_paramters($band)
{
	...
	else if ($band == "g")	echo "APTxop=0;0;94;47"."\n";
	echo "APACM=0;0;0;0"	."\n";
	...
	else if ($band == "g")	echo "BSSTxop=0;0;94;47"	."\n";
	echo "BSSACM=0;0;0;0"		."\n";
...
function mbssid_paramters($phy_uid)
{
	...
	foreach ("/phyinf")
	{
		if (query("uid")==$phy_uid || query("media/parent")==$phy_uid)
		{
			...
			else if ($auth == "WPA2PSK")		$authtype = $authtype.$delimiter."WPA1WPA2";
			else if ($auth == "WPA+2")			$authtype = $authtype.$delimiter."WPAWPA2";
...
function wds_parameters()
{
	...
	foreach ("/phyinf")
	{
		if (cut(query("uid"), 0, "-") == "WDS")
		{
			...
			$peermac = query("media/peermac");
			if ($peermac != "")
			{
				if ($idx == 0)  $delimiter = "";
				else            $delimiter = ";";

                if      ($encrtype == "NONE")        $encrtypes = $encrtypes.$delimiter."NONE";
				else if ($encrtype == "WEP")         $encrtypes = $encrtypes.$delimiter."WEP";
				else if ($encrtype == "TKIP")        $encrtypes = $encrtypes.$delimiter."TKIP";
				else if ($encrtype == "AES")         $encrtypes = $encrtypes.$delimiter."AES";
				else if ($encrtype == "TKIP+AES")    $encrtypes = $encrtypes.$delimiter."AES";
				if ($encrtype == "WEP")
				{
					$defkeyId = query($wifi."/nwkey/wep/defkey");
					$defkeyIds = $defkeyIds.$delimiter.$defkeyId;
					$wdskey = query($wifi."/nwkey/wep/key:".$defkeyId);
				}
				else
				{
					$wdskey = query($wifi."/nwkey/psk/key");
				}

				$wdslist = $wdslist.$delimiter.$peermac;
				$wdsphymode = $wdsphymode.$delimiter."HTMIX";
				echo "Wds".$idx."Key=".$wdskey."\n";
				$idx++;
			}
		}
	}

	if ($idx > 0) 	echo "WdsEnable=3\n";
	else			echo "WdsEnable=0\n";

	if ($defkeyIds == "") $defkeyIds = 0;	

	echo "WdsDefaultKeyID="	.$defkeyIds."\n";
	echo "WdsPhyMode="		.$wdsphymode."\n";
	echo "WdsEncrypType="	.$encrtypes."\n";
...
if		($ccode == "JP") {$c_region = 1;}
else if	($ccode == "GB") {$c_region = 1;}
else if ($ccode == "CL") {$c_region = 1;}
...
if ($en11n == 0)
{
	$txrate = query($phy."/meadia/txrate");
